import React from "react";
import saltImage from "../../assets/media/saltimage.png";
import chip2 from "../../assets/media/chip2.png";
import chip3 from "../../assets/media/chip3.png";

const NameSearchPageDemo = () => {
  return (
    <section>
      <div className="product-listing_container___XHEM">
        <div className="product-listing_chipsContainer__4nBcx">
          <div className="product-listing_chip__uK7Zr">
            <img
              alt=""
              loading="lazy"
              width="32"
              height="32"
              decoding="async"
              className="product-listing_chipIcon__RWzS2"
              style={{ color: "transparent" }}
              srcSet={`
                ${saltImage}?w=32&amp;q=80 1x,
                ${saltImage}?w=64&amp;q=80 2x
              `}
              src={`${saltImage}?w=64&amp;q=80`}
            />
            <p>
              Exact Same
              <br />
              Composition
            </p>
          </div>
          <div className="product-listing_chip__uK7Zr">
            <img
              alt=""
              loading="lazy"
              width="32"
              height="32"
              decoding="async"
              className="product-listing_chipIcon__RWzS2"
              style={{ color: "transparent" }}
              srcSet={`
               ${chip2}?w=32&amp;q=80 1x,
                ${chip2}?w=64&amp;q=80 2x
              `}
              src={chip2}
            />
            <p>
              Upto 60%
              <br />
              Savings
            </p>
          </div>
          <div className="product-listing_chip__uK7Zr">
            <img
              alt=""
              loading="lazy"
              width="32"
              height="32"
              decoding="async"
              className="product-listing_chipIcon__RWzS2"
              style={{ color: "transparent" }}
              srcSet={`
                ${chip3}?w=32&amp;q=80 1x,
                ${chip3}?w=64&amp;q=80 2x
              `}
              src="/_next/static/media/chip3.ae944398.png?w=64&amp;q=80"
            />
            <p>
              Doctor
              <br />
              Approved
            </p>
          </div>
        </div>
        <div className="product-listing_stickyBanner__SMXnr">
          <div className="product-listing_RowHeader__xPwBq">
            <div className="product-listing_RowLeft__KA_kE">You Searched</div>
            <div className="product-listing_RowRight__hlV7c">
              Our Recommendation
            </div>
          </div>
        </div>
        <div>
          <a href="/product-display/dolo-150mg-injection/1416278">
            <div className="druglist-item_itemContainer__Uqt1t">
              <div className="druglist-item_itemHeading__cWGyl">
                <div className="druglist-item_saltMobileContainer__0R4yP">
                  <div className="druglist-item_saltMobileContainer__content__ILi6H">
                    <div className="druglist-item_saltMobileContainer__title__3zs9s">
                      <img
                        alt="Molecule"
                        loading="lazy"
                        width="32"
                        height="32"
                        decoding="async"
                        className="druglist-item_saltMobileContainer__icon__L13mN"
                        srcSet={`
                            ${saltImage}?w=32&amp;q=80 1x,
                            ${saltImage}?w=64&amp;q=80 2x
                          `}
                        src={`${saltImage}?w=64&amp;q=80`}
                        style={{ color: "transparent", display: "none" }}
                      />
                      <div className="druglist-item_saltMobileContainer__titleText__9UmPF">
                        Salt Composition in both
                      </div>
                    </div>
                    <div className="druglist-item_saltMobileContainer__composition__EsLOi">
                      <p className="druglist-item_saltMobileContainer__composition__less_salt__B_Gip">
                        Paracetamol (Acetaminophen) 150mg
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="druglist-item_fullDetailsContainer__RCxNm">
                <div className="druglist-item_genericMedicineDetails__rdmT5">
                  <div
                    style={{
                      display: "flex",
                      justifyContent: "space-between",
                      flexDirection: "row",
                    }}
                  >
                    <div className="druglist-item_contentContainer__OyI1B">
                      <div
                        className="druglist-item_imgContainer__3MOmx"
                        style={{
                          display: "flex",
                          alignItems: "flex-end",
                          flexWrap: "nowrap",
                        }}
                      >
                        <div
                          className="druglist-item_itemImg__KBvIo"
                          style={{
                            backgroundImage:
                              "url('https://platinumrxassets.blob.core.windows.net/platinumrx-assets/default_image.jpg')",
                          }}
                        ></div>
                      </div>
                      <div
                        className="druglist-item_itemDetails__JhI3f"
                        style={{ marginTop: "8px" }}
                      >
                        <div className="druglist-item_itemName__YnmLq">
                          Dolo 150mg Injection
                        </div>
                        <div className="druglist-item_genericManufacturerName__Du3NS">
                          Micro Labs Ltd
                        </div>
                        <div className="druglist-item_Quantity__y1rmm">
                          Vial of 2 ml
                        </div>
                        <div className="druglist-item_itemMrp__aSi9J">MRP</div>
                        <p className="druglist-item_genericPrice__0u9Ol">
                          ₹9.8
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
                <div
                  className="druglist-item_substituteHeaderContainer__ezxIX"
                  style={{ width: "65%" }}
                >
                  <div className="druglist-item_substituteMedicineDetails__0wLPq">
                    <div
                      style={{
                        display: "flex",
                        justifyContent: "space-between",
                        flexDirection: "row",
                      }}
                    >
                      <div className="druglist-item_contentContainer__OyI1B">
                        <div
                          className="druglist-item_imgContainer__3MOmx"
                          style={{
                            display: "flex",
                            alignItems: "flex-end",
                            flexWrap: "nowrap",
                          }}
                        >
                          <div
                            className="druglist-item_itemImg__KBvIo"
                            style={{
                              backgroundImage:
                                "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                            }}
                          ></div>
                        </div>
                        <div
                          className="druglist-item_itemDetails__JhI3f"
                          style={{ marginTop: "8px" }}
                        >
                          <div className="druglist-item_itemName__YnmLq">
                            Pyricool 150mg Injection 2ml
                          </div>
                          <div className="druglist-item_substituteManufacturerName__fECQK">
                            Alkem Laboratories Ltd
                          </div>
                          <div className="druglist-item_Quantity__y1rmm">
                            vial of 2 ml
                          </div>
                          <div
                            className="druglist-item_addtoCartContainer__0XkUC"
                            style={{
                              display: "flex",
                              justifyContent: "space-between",
                              alignItems: "flex-end",
                            }}
                          >
                            <div className="druglist-item_substitutePriceContainer__75Y7h">
                              <div className="druglist-item_substitutePrice__Qdsht">
                                <p className="druglist-item_discountedPrice__ftOvo">
                                  ₹7.1
                                </p>
                              </div>
                              <p className="druglist-item_itemMrp__aSi9J">
                                MRP ₹<span>8.9</span>
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          justifyContent: "flex-end",
                          alignItems: "flex-end",
                          position: "relative",
                        }}
                      >
                        <div className="druglist-item_SavingLabel__B_Smj">
                          27% Savings
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </a>
          <div className="product-listing_divider__aiMpd"></div>
          <a href="/product-display/dolo-500-tablet-10-tablet/1035713">
            <div className="druglist-item_itemContainer__Uqt1t">
              <div className="druglist-item_itemHeading__cWGyl">
                <div className="druglist-item_saltMobileContainer__0R4yP">
                  <div className="druglist-item_saltMobileContainer__content__ILi6H">
                    <div className="druglist-item_saltMobileContainer__title__3zs9s">
                      <img
                        alt="Molecule"
                        loading="lazy"
                        width="32"
                        height="32"
                        decoding="async"
                        className="druglist-item_saltMobileContainer__icon__L13mN"
                        srcSet={`
                            ${saltImage}?w=32&amp;q=80 1x,
                            ${saltImage}?w=64&amp;q=80 2x
                          `}
                        src={`${saltImage}?w=64&amp;q=80`}
                        style={{ color: "transparent", display: "none" }}
                      />
                      <div className="druglist-item_saltMobileContainer__titleText__9UmPF">
                        Salt Composition in both
                      </div>
                    </div>
                    <div className="druglist-item_saltMobileContainer__composition__EsLOi">
                      <p className="druglist-item_saltMobileContainer__composition__less_salt__B_Gip">
                        Paracetamol (Acetaminophen) 500mg
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="druglist-item_fullDetailsContainer__RCxNm">
                <div className="druglist-item_genericMedicineDetails__rdmT5">
                  <div
                    style={{
                      display: "flex",
                      justifyContent: "space-between",
                      flexDirection: "row",
                    }}
                  >
                    <div className="druglist-item_contentContainer__OyI1B">
                      <div
                        className="druglist-item_imgContainer__3MOmx"
                        style={{
                          display: "flex",
                          alignItems: "flex-end",
                          flexWrap: "nowrap",
                        }}
                      >
                        <div
                          className="druglist-item_itemImg__KBvIo"
                          style={{
                            backgroundImage:
                              "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/e9ac609e-3447-4afd-baea-ac035584a551_wm.png')",
                          }}
                        ></div>
                      </div>
                      <div
                        className="druglist-item_itemDetails__JhI3f"
                        style={{ marginTop: "8px" }}
                      >
                        <div className="druglist-item_itemName__YnmLq">
                          Dolo 500 Tablet 10 Tablet
                        </div>
                        <div className="druglist-item_genericManufacturerName__Du3NS">
                          Micro Labs Ltd
                        </div>
                        <div className="druglist-item_Quantity__y1rmm">
                          Strip of 10 tablets
                        </div>
                        <div className="druglist-item_itemMrp__aSi9J">MRP</div>
                        <p className="druglist-item_genericPrice__0u9Ol">
                          ₹10.2
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
                <div
                  className="druglist-item_substituteHeaderContainer__ezxIX"
                  style={{ width: "65%" }}
                >
                  <div className="druglist-item_substituteMedicineDetails__0wLPq">
                    <div
                      style={{
                        display: "flex",
                        justifyContent: "space-between",
                        flexDirection: "row",
                      }}
                    >
                      <div className="druglist-item_contentContainer__OyI1B">
                        <div
                          className="druglist-item_imgContainer__3MOmx"
                          style={{
                            display: "flex",
                            alignItems: "flex-end",
                            flexWrap: "nowrap",
                          }}
                        >
                          <div
                            className="druglist-item_itemImg__KBvIo"
                            style={{
                              backgroundImage:
                                "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                            }}
                          ></div>
                        </div>
                        <div
                          className="druglist-item_itemDetails__JhI3f"
                          style={{ marginTop: "8px" }}
                        >
                          <div className="druglist-item_itemName__YnmLq">
                            Paracip 500mg Tab 10s
                          </div>
                          <div className="druglist-item_substituteManufacturerName__fECQK">
                            Cipla Ltd
                          </div>
                          <div className="druglist-item_Quantity__y1rmm">
                            Strip of 10 tablets
                          </div>
                          <div
                            className="druglist-item_addtoCartContainer__0XkUC"
                            style={{
                              display: "flex",
                              justifyContent: "space-between",
                              alignItems: "flex-end",
                            }}
                          >
                            <div className="druglist-item_substitutePriceContainer__75Y7h">
                              <div className="druglist-item_substitutePrice__Qdsht">
                                <p className="druglist-item_discountedPrice__ftOvo">
                                  ₹7.9
                                </p>
                              </div>
                              <p className="druglist-item_itemMrp__aSi9J">
                                MRP ₹<span>10.1</span>
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          justifyContent: "flex-end",
                          alignItems: "flex-end",
                          position: "relative",
                        }}
                      >
                        <div className="druglist-item_SavingLabel__B_Smj">
                          23% Savings
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </a>
          <div className="product-listing_divider__aiMpd"></div>
          <a href="/product-display/dolo-mf-junior-oral-suspension/1224812">
            <div className="druglist-item_itemContainer__Uqt1t">
              <div className="druglist-item_itemHeading__cWGyl">
                <div className="druglist-item_saltMobileContainer__0R4yP">
                  <div className="druglist-item_saltMobileContainer__content__ILi6H">
                    <div className="druglist-item_saltMobileContainer__title__3zs9s">
                      <img
                        alt="Molecule"
                        loading="lazy"
                        width="32"
                        height="32"
                        decoding="async"
                        className="druglist-item_saltMobileContainer__icon__L13mN"
                        srcSet={`
                            ${saltImage}?w=32&amp;q=80 1x,
                            ${saltImage}?w=64&amp;q=80 2x
                          `}
                        src={`${saltImage}?w=64&amp;q=80`}
                        style={{ color: "transparent", display: "none" }}
                      />
                      <div className="druglist-item_saltMobileContainer__titleText__9UmPF">
                        Salt Composition in both
                      </div>
                    </div>
                    <div className="druglist-item_saltMobileContainer__composition__EsLOi">
                      <p className="druglist-item_saltMobileContainer__composition__less_salt__B_Gip">
                        Mefenamic Acid 50mg + Paracetamol (Acetaminophen) 125mg
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="druglist-item_fullDetailsContainer__RCxNm">
                <div className="druglist-item_genericMedicineDetails__rdmT5">
                  <div
                    style={{
                      display: "flex",
                      justifyContent: "space-between",
                      flexDirection: "row",
                    }}
                  >
                    <div className="druglist-item_contentContainer__OyI1B">
                      <div
                        className="druglist-item_imgContainer__3MOmx"
                        style={{
                          display: "flex",
                          alignItems: "flex-end",
                          flexWrap: "nowrap",
                        }}
                      >
                        <div
                          className="druglist-item_itemImg__KBvIo"
                          style={{
                            backgroundImage:
                              "url('https://platinumrxassets.blob.core.windows.net/platinumrx-assets/default_image.jpg')",
                          }}
                        ></div>
                      </div>
                      <div
                        className="druglist-item_itemDetails__JhI3f"
                        style={{ marginTop: "8px" }}
                      >
                        <div className="druglist-item_itemName__YnmLq">
                          Dolo-MF Junior Oral Suspension
                        </div>
                        <div className="druglist-item_genericManufacturerName__Du3NS">
                          Micro Labs Ltd
                        </div>
                        <div className="druglist-item_Quantity__y1rmm">
                          Bottle of 60 ml
                        </div>
                        <div className="druglist-item_itemMrp__aSi9J">MRP</div>
                        <p className="druglist-item_genericPrice__0u9Ol">₹63</p>
                      </div>
                    </div>
                  </div>
                </div>
                <div
                  className="druglist-item_substituteHeaderContainer__ezxIX"
                  style={{ width: "65%" }}
                >
                  <div className="druglist-item_substituteMedicineDetails__0wLPq">
                    <div
                      style={{
                        display: "flex",
                        justifyContent: "space-between",
                        flexDirection: "row",
                      }}
                    >
                      <div className="druglist-item_contentContainer__OyI1B">
                        <div
                          className="druglist-item_imgContainer__3MOmx"
                          style={{
                            display: "flex",
                            alignItems: "flex-end",
                            flexWrap: "nowrap",
                          }}
                        >
                          <div
                            className="druglist-item_itemImg__KBvIo"
                            style={{
                              backgroundImage:
                                "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                            }}
                          ></div>
                        </div>
                        <div
                          className="druglist-item_itemDetails__JhI3f"
                          style={{ marginTop: "8px" }}
                        >
                          <div className="druglist-item_itemName__YnmLq">
                            Mefpen Plus 50/125mg Oral Suspension 60ml
                          </div>
                          <div className="druglist-item_substituteManufacturerName__fECQK">
                            Dr. Morepen
                          </div>
                          <div className="druglist-item_Quantity__y1rmm">
                            Bottle of 60 ml
                          </div>
                          <div
                            className="druglist-item_addtoCartContainer__0XkUC"
                            style={{
                              display: "flex",
                              justifyContent: "space-between",
                              alignItems: "flex-end",
                            }}
                          >
                            <div className="druglist-item_substitutePriceContainer__75Y7h">
                              <div className="druglist-item_substitutePrice__Qdsht">
                                <p className="druglist-item_discountedPrice__ftOvo">
                                  ₹39.5
                                </p>
                              </div>
                              <p className="druglist-item_itemMrp__aSi9J">
                                MRP ₹<span>79</span>
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          justifyContent: "flex-end",
                          alignItems: "flex-end",
                          position: "relative",
                        }}
                      >
                        <div className="druglist-item_SavingLabel__B_Smj">
                          37% Savings
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </a>
          <div className="product-listing_divider__aiMpd"></div>
          <a href="/product-display/dolo-250-oral-suspension/1301758">
            <div className="druglist-item_itemContainer__Uqt1t">
              <div className="druglist-item_itemHeading__cWGyl">
                <div className="druglist-item_saltMobileContainer__0R4yP">
                  <div className="druglist-item_saltMobileContainer__content__ILi6H">
                    <div className="druglist-item_saltMobileContainer__title__3zs9s">
                      <img
                        alt="Molecule"
                        loading="lazy"
                        width="32"
                        height="32"
                        decoding="async"
                        className="druglist-item_saltMobileContainer__icon__L13mN"
                        srcSet={`
                            ${saltImage}?w=32&amp;q=80 1x,
                            ${saltImage}?w=64&amp;q=80 2x
                          `}
                        src={`${saltImage}?w=64&amp;q=80`}
                        style={{ color: "transparent", display: "none" }}
                      />
                      <div className="druglist-item_saltMobileContainer__titleText__9UmPF">
                        Salt Composition in both
                      </div>
                    </div>
                    <div className="druglist-item_saltMobileContainer__composition__EsLOi">
                      <p className="druglist-item_saltMobileContainer__composition__less_salt__B_Gip">
                        Paracetamol (Acetaminophen) 250mg
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="druglist-item_fullDetailsContainer__RCxNm">
                <div className="druglist-item_genericMedicineDetails__rdmT5">
                  <div
                    style={{
                      display: "flex",
                      justifyContent: "space-between",
                      flexDirection: "row",
                    }}
                  >
                    <div className="druglist-item_contentContainer__OyI1B">
                      <div
                        className="druglist-item_imgContainer__3MOmx"
                        style={{
                          display: "flex",
                          alignItems: "flex-end",
                          flexWrap: "nowrap",
                        }}
                      >
                        <div
                          className="druglist-item_itemImg__KBvIo"
                          style={{
                            backgroundImage:
                              "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                          }}
                        ></div>
                      </div>
                      <div
                        className="druglist-item_itemDetails__JhI3f"
                        style={{ marginTop: "8px" }}
                      >
                        <div className="druglist-item_itemName__YnmLq">
                          Dolo 250 Oral Suspension
                        </div>
                        <div className="druglist-item_genericManufacturerName__Du3NS">
                          Micro Labs Ltd
                        </div>
                        <div className="druglist-item_Quantity__y1rmm">
                          Bottle of 60 ml
                        </div>
                        <div className="druglist-item_itemMrp__aSi9J">MRP</div>
                        <p className="druglist-item_genericPrice__0u9Ol">₹45</p>
                      </div>
                    </div>
                  </div>
                </div>
                <div
                  className="druglist-item_substituteHeaderContainer__ezxIX"
                  style={{ width: "65%" }}
                >
                  <div className="druglist-item_substituteMedicineDetails__0wLPq">
                    <div
                      style={{
                        display: "flex",
                        justifyContent: "space-between",
                        flexDirection: "row",
                      }}
                    >
                      <div className="druglist-item_contentContainer__OyI1B">
                        <div
                          className="druglist-item_imgContainer__3MOmx"
                          style={{
                            display: "flex",
                            alignItems: "flex-end",
                            flexWrap: "nowrap",
                          }}
                        >
                          <div
                            className="druglist-item_itemImg__KBvIo"
                            style={{
                              backgroundImage:
                                "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                            }}
                          ></div>
                        </div>
                        <div
                          className="druglist-item_itemDetails__JhI3f"
                          style={{ marginTop: "8px" }}
                        >
                          <div className="druglist-item_itemName__YnmLq">
                            Paracip 250mg Syrup 60ml
                          </div>
                          <div className="druglist-item_substituteManufacturerName__fECQK">
                            Cipla Ltd
                          </div>
                          <div className="druglist-item_Quantity__y1rmm">
                            Bottle of 60 ml
                          </div>
                          <div
                            className="druglist-item_addtoCartContainer__0XkUC"
                            style={{
                              display: "flex",
                              justifyContent: "space-between",
                              alignItems: "flex-end",
                            }}
                          >
                            <div className="druglist-item_substitutePriceContainer__75Y7h">
                              <div className="druglist-item_substitutePrice__Qdsht">
                                <p className="druglist-item_discountedPrice__ftOvo">
                                  ₹25.2
                                </p>
                              </div>
                              <p className="druglist-item_itemMrp__aSi9J">
                                MRP ₹<span>45</span>
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          justifyContent: "flex-end",
                          alignItems: "flex-end",
                          position: "relative",
                        }}
                      >
                        <div className="druglist-item_SavingLabel__B_Smj">
                          44% Savings
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </a>
          <div className="product-listing_divider__aiMpd"></div>
          <a href="/product-display/dolo-sp-500mg-tablet/1311154">
            <div className="druglist-item_itemContainer__Uqt1t">
              <div className="druglist-item_itemHeading__cWGyl">
                <div className="druglist-item_saltMobileContainer__0R4yP">
                  <div className="druglist-item_saltMobileContainer__content__ILi6H">
                    <div className="druglist-item_saltMobileContainer__title__3zs9s">
                      <img
                        alt="Molecule"
                        loading="lazy"
                        width="32"
                        height="32"
                        decoding="async"
                        className="druglist-item_saltMobileContainer__icon__L13mN"
                        srcSet={`
                            ${saltImage}?w=32&amp;q=80 1x,
                            ${saltImage}?w=64&amp;q=80 2x
                          `}
                        src={`${saltImage}?w=64&amp;q=80`}
                        style={{ color: "transparent", display: "none" }}
                      />
                      <div className="druglist-item_saltMobileContainer__titleText__9UmPF">
                        Salt Composition in both
                      </div>
                    </div>
                    <div className="druglist-item_saltMobileContainer__composition__EsLOi">
                      <p className="druglist-item_saltMobileContainer__composition__less_salt__B_Gip">
                        Paracetamol (Acetaminophen) 500mg
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="druglist-item_fullDetailsContainer__RCxNm">
                <div className="druglist-item_genericMedicineDetails__rdmT5">
                  <div
                    style={{
                      display: "flex",
                      justifyContent: "space-between",
                      flexDirection: "row",
                    }}
                  >
                    <div className="druglist-item_contentContainer__OyI1B">
                      <div
                        className="druglist-item_imgContainer__3MOmx"
                        style={{
                          display: "flex",
                          alignItems: "flex-end",
                          flexWrap: "nowrap",
                        }}
                      >
                        <div
                          className="druglist-item_itemImg__KBvIo"
                          style={{
                            backgroundImage:
                              "url('https://platinumrxassets.blob.core.windows.net/platinumrx-assets/default_image.jpg')",
                          }}
                        ></div>
                      </div>
                      <div
                        className="druglist-item_itemDetails__JhI3f"
                        style={{ marginTop: "8px" }}
                      >
                        <div className="druglist-item_itemName__YnmLq">
                          Dolo SP 500mg Tablet
                        </div>
                        <div className="druglist-item_genericManufacturerName__Du3NS">
                          Bioguard Organics Ltd
                        </div>
                        <div className="druglist-item_Quantity__y1rmm">
                          Strip of 10 tablets
                        </div>
                        <div className="druglist-item_itemMrp__aSi9J">MRP</div>
                        <p className="druglist-item_genericPrice__0u9Ol">
                          ₹68.8
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
                <div
                  className="druglist-item_substituteHeaderContainer__ezxIX"
                  style={{ width: "65%" }}
                >
                  <div className="druglist-item_substituteMedicineDetails__0wLPq">
                    <div
                      style={{
                        display: "flex",
                        justifyContent: "space-between",
                        flexDirection: "row",
                      }}
                    >
                      <div className="druglist-item_contentContainer__OyI1B">
                        <div
                          className="druglist-item_imgContainer__3MOmx"
                          style={{
                            display: "flex",
                            alignItems: "flex-end",
                            flexWrap: "nowrap",
                          }}
                        >
                          <div
                            className="druglist-item_itemImg__KBvIo"
                            style={{
                              backgroundImage:
                                "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                            }}
                          ></div>
                        </div>
                        <div
                          className="druglist-item_itemDetails__JhI3f"
                          style={{ marginTop: "8px" }}
                        >
                          <div className="druglist-item_itemName__YnmLq">
                            Paracip 500mg Tab 10s
                          </div>
                          <div className="druglist-item_substituteManufacturerName__fECQK">
                            Cipla Ltd
                          </div>
                          <div className="druglist-item_Quantity__y1rmm">
                            Strip of 10 tablets
                          </div>
                          <div
                            className="druglist-item_addtoCartContainer__0XkUC"
                            style={{
                              display: "flex",
                              justifyContent: "space-between",
                              alignItems: "flex-end",
                            }}
                          >
                            <div className="druglist-item_substitutePriceContainer__75Y7h">
                              <div className="druglist-item_substitutePrice__Qdsht">
                                <p className="druglist-item_discountedPrice__ftOvo">
                                  ₹7.9
                                </p>
                              </div>
                              <p className="druglist-item_itemMrp__aSi9J">
                                MRP ₹<span>10.1</span>
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          justifyContent: "flex-end",
                          alignItems: "flex-end",
                          position: "relative",
                        }}
                      >
                        <div className="druglist-item_SavingLabel__B_Smj">
                          89% Savings
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </a>
          <div className="product-listing_divider__aiMpd"></div>
          <a href="/product-display/dolo-650mg-tab-10-s/1309803">
            <div className="druglist-item_itemContainer__Uqt1t">
              <div className="druglist-item_itemHeading__cWGyl">
                <div className="druglist-item_saltMobileContainer__0R4yP">
                  <div className="druglist-item_saltMobileContainer__content__ILi6H">
                    <div className="druglist-item_saltMobileContainer__title__3zs9s">
                      <img
                        alt="Molecule"
                        loading="lazy"
                        width="32"
                        height="32"
                        decoding="async"
                        className="druglist-item_saltMobileContainer__icon__L13mN"
                        srcSet={`
                            ${saltImage}?w=32&amp;q=80 1x,
                            ${saltImage}?w=64&amp;q=80 2x
                          `}
                        src={`${saltImage}?w=64&amp;q=80`}
                        style={{ color: "transparent", display: "none" }}
                      />
                      <div className="druglist-item_saltMobileContainer__titleText__9UmPF">
                        Salt Composition in both
                      </div>
                    </div>
                    <div className="druglist-item_saltMobileContainer__composition__EsLOi">
                      <p className="druglist-item_saltMobileContainer__composition__less_salt__B_Gip">
                        Paracetamol (Acetaminophen) 650mg
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="druglist-item_fullDetailsContainer__RCxNm">
                <div className="druglist-item_genericMedicineDetails__rdmT5">
                  <div
                    style={{
                      display: "flex",
                      justifyContent: "space-between",
                      flexDirection: "row",
                    }}
                  >
                    <div className="druglist-item_contentContainer__OyI1B">
                      <div
                        className="druglist-item_imgContainer__3MOmx"
                        style={{
                          display: "flex",
                          alignItems: "flex-end",
                          flexWrap: "nowrap",
                        }}
                      >
                        <div
                          className="druglist-item_itemImg__KBvIo"
                          style={{
                            backgroundImage:
                              "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                          }}
                        ></div>
                      </div>
                      <div
                        className="druglist-item_itemDetails__JhI3f"
                        style={{ marginTop: "8px" }}
                      >
                        <div className="druglist-item_itemName__YnmLq">
                          Dolo 650mg Tab 10's
                        </div>
                        <div className="druglist-item_genericManufacturerName__Du3NS">
                          Micro Labs Ltd
                        </div>
                        <div className="druglist-item_Quantity__y1rmm">
                          Strip of 10 tablets
                        </div>
                        <div className="druglist-item_itemMrp__aSi9J">MRP</div>
                        <p className="druglist-item_genericPrice__0u9Ol">
                          ₹19.9
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
                <div
                  className="druglist-item_substituteHeaderContainer__ezxIX"
                  style={{ width: "65%" }}
                >
                  <div className="druglist-item_substituteMedicineDetails__0wLPq">
                    <div
                      style={{
                        display: "flex",
                        justifyContent: "space-between",
                        flexDirection: "row",
                      }}
                    >
                      <div className="druglist-item_contentContainer__OyI1B">
                        <div
                          className="druglist-item_imgContainer__3MOmx"
                          style={{
                            display: "flex",
                            alignItems: "flex-end",
                            flexWrap: "nowrap",
                          }}
                        >
                          <div
                            className="druglist-item_itemImg__KBvIo"
                            style={{
                              backgroundImage:
                                "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                            }}
                          ></div>
                        </div>
                        <div
                          className="druglist-item_itemDetails__JhI3f"
                          style={{ marginTop: "8px" }}
                        >
                          <div className="druglist-item_itemName__YnmLq">
                            Paracip 650mg Tab 10s
                          </div>
                          <div className="druglist-item_substituteManufacturerName__fECQK">
                            Cipla Ltd
                          </div>
                          <div className="druglist-item_Quantity__y1rmm">
                            Strip of 10 tablets
                          </div>
                          <div
                            className="druglist-item_addtoCartContainer__0XkUC"
                            style={{
                              display: "flex",
                              justifyContent: "space-between",
                              alignItems: "flex-end",
                            }}
                          >
                            <div className="druglist-item_substitutePriceContainer__75Y7h">
                              <div className="druglist-item_substitutePrice__Qdsht">
                                <p className="druglist-item_discountedPrice__ftOvo">
                                  ₹13.5
                                </p>
                              </div>
                              <p className="druglist-item_itemMrp__aSi9J">
                                MRP ₹<span>22.5</span>
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          justifyContent: "flex-end",
                          alignItems: "flex-end",
                          position: "relative",
                        }}
                      >
                        <div className="druglist-item_SavingLabel__B_Smj">
                          32% Savings
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </a>
          <div className="product-listing_divider__aiMpd"></div>
          <a href="/product-display/dolo-650mg-tab-15-s/1111190">
            <div className="druglist-item_itemContainer__Uqt1t">
              <div className="druglist-item_itemHeading__cWGyl">
                <div className="druglist-item_saltMobileContainer__0R4yP">
                  <div className="druglist-item_saltMobileContainer__content__ILi6H">
                    <div className="druglist-item_saltMobileContainer__title__3zs9s">
                      <img
                        alt="Molecule"
                        loading="lazy"
                        width="32"
                        height="32"
                        decoding="async"
                        className="druglist-item_saltMobileContainer__icon__L13mN"
                        srcSet={`
                            ${saltImage}?w=32&amp;q=80 1x,
                            ${saltImage}?w=64&amp;q=80 2x
                          `}
                        src={`${saltImage}?w=64&amp;q=80`}
                        style={{ color: "transparent", display: "none" }}
                      />
                      <div className="druglist-item_saltMobileContainer__titleText__9UmPF">
                        Salt Composition in both
                      </div>
                    </div>
                    <div className="druglist-item_saltMobileContainer__composition__EsLOi">
                      <p className="druglist-item_saltMobileContainer__composition__less_salt__B_Gip">
                        Paracetamol (Acetaminophen) 650mg
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="druglist-item_fullDetailsContainer__RCxNm">
                <div className="druglist-item_genericMedicineDetails__rdmT5">
                  <div
                    style={{
                      display: "flex",
                      justifyContent: "space-between",
                      flexDirection: "row",
                    }}
                  >
                    <div className="druglist-item_contentContainer__OyI1B">
                      <div
                        className="druglist-item_imgContainer__3MOmx"
                        style={{
                          display: "flex",
                          alignItems: "flex-end",
                          flexWrap: "nowrap",
                        }}
                      >
                        <div
                          className="druglist-item_itemImg__KBvIo"
                          style={{
                            backgroundImage:
                              "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                          }}
                        ></div>
                      </div>
                      <div
                        className="druglist-item_itemDetails__JhI3f"
                        style={{ marginTop: "8px" }}
                      >
                        <div className="druglist-item_itemName__YnmLq">
                          Dolo 650mg Tab 15's
                        </div>
                        <div className="druglist-item_genericManufacturerName__Du3NS">
                          Micro Labs Ltd
                        </div>
                        <div className="druglist-item_Quantity__y1rmm">
                          Strip of 15 tablets
                        </div>
                        <div className="druglist-item_itemMrp__aSi9J">MRP</div>
                        <p className="druglist-item_genericPrice__0u9Ol">
                          ₹33.8
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
                <div
                  className="druglist-item_substituteHeaderContainer__ezxIX"
                  style={{ width: "65%" }}
                >
                  <div className="druglist-item_substituteMedicineDetails__0wLPq">
                    <div
                      style={{
                        display: "flex",
                        justifyContent: "space-between",
                        flexDirection: "row",
                      }}
                    >
                      <div className="druglist-item_contentContainer__OyI1B">
                        <div
                          className="druglist-item_imgContainer__3MOmx"
                          style={{
                            display: "flex",
                            alignItems: "flex-end",
                            flexWrap: "nowrap",
                          }}
                        >
                          <div
                            className="druglist-item_itemImg__KBvIo"
                            style={{
                              backgroundImage:
                                "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                            }}
                          ></div>
                        </div>
                        <div
                          className="druglist-item_itemDetails__JhI3f"
                          style={{ marginTop: "8px" }}
                        >
                          <div className="druglist-item_itemName__YnmLq">
                            Paracip 650mg Tab 10s
                          </div>
                          <div className="druglist-item_substituteManufacturerName__fECQK">
                            Cipla Ltd
                          </div>
                          <div className="druglist-item_Quantity__y1rmm">
                            Strip of 10 tablets
                          </div>
                          <div
                            className="druglist-item_addtoCartContainer__0XkUC"
                            style={{
                              display: "flex",
                              justifyContent: "space-between",
                              alignItems: "flex-end",
                            }}
                          >
                            <div className="druglist-item_substitutePriceContainer__75Y7h">
                              <div className="druglist-item_substitutePrice__Qdsht">
                                <p className="druglist-item_discountedPrice__ftOvo">
                                  ₹13.5
                                </p>
                              </div>
                              <p className="druglist-item_itemMrp__aSi9J">
                                MRP ₹<span>22.5</span>
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          justifyContent: "flex-end",
                          alignItems: "flex-end",
                          position: "relative",
                        }}
                      >
                        <div className="druglist-item_SavingLabel__B_Smj">
                          40% Savings
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </a>
          <div className="product-listing_divider__aiMpd"></div>
          <a href="/product-display/dolo-1000mg-infusion/1264423">
            <div className="druglist-item_itemContainer__Uqt1t">
              <div className="druglist-item_itemHeading__cWGyl">
                <div className="druglist-item_saltMobileContainer__0R4yP">
                  <div className="druglist-item_saltMobileContainer__content__ILi6H">
                    <div className="druglist-item_saltMobileContainer__title__3zs9s">
                      <img
                        alt="Molecule"
                        loading="lazy"
                        width="32"
                        height="32"
                        decoding="async"
                        className="druglist-item_saltMobileContainer__icon__L13mN"
                        srcSet={`
                            ${saltImage}?w=32&amp;q=80 1x,
                            ${saltImage}?w=64&amp;q=80 2x
                          `}
                        src={`${saltImage}?w=64&amp;q=80`}
                        style={{ color: "transparent", display: "none" }}
                      />
                      <div className="druglist-item_saltMobileContainer__titleText__9UmPF">
                        Salt Composition in both
                      </div>
                    </div>
                    <div className="druglist-item_saltMobileContainer__composition__EsLOi">
                      <p className="druglist-item_saltMobileContainer__composition__less_salt__B_Gip">
                        Paracetamol (Acetaminophen) 1000mg
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="druglist-item_fullDetailsContainer__RCxNm">
                <div className="druglist-item_genericMedicineDetails__rdmT5">
                  <div
                    style={{
                      display: "flex",
                      justifyContent: "space-between",
                      flexDirection: "row",
                    }}
                  >
                    <div className="druglist-item_contentContainer__OyI1B">
                      <div
                        className="druglist-item_imgContainer__3MOmx"
                        style={{
                          display: "flex",
                          alignItems: "flex-end",
                          flexWrap: "nowrap",
                        }}
                      >
                        <div
                          className="druglist-item_itemImg__KBvIo"
                          style={{
                            backgroundImage:
                              "url('https://platinumrxassets.blob.core.windows.net/platinumrx-assets/default_image.jpg')",
                          }}
                        ></div>
                      </div>
                      <div
                        className="druglist-item_itemDetails__JhI3f"
                        style={{ marginTop: "8px" }}
                      >
                        <div className="druglist-item_itemName__YnmLq">
                          Dolo 1000mg Infusion
                        </div>
                        <div className="druglist-item_genericManufacturerName__Du3NS">
                          Micro Labs Ltd
                        </div>
                        <div className="druglist-item_Quantity__y1rmm">
                          Bottle of 100 ml
                        </div>
                        <div className="druglist-item_itemMrp__aSi9J">MRP</div>
                        <p className="druglist-item_genericPrice__0u9Ol">
                          ₹268
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
                <div
                  className="druglist-item_substituteHeaderContainer__ezxIX"
                  style={{ width: "65%" }}
                >
                  <div className="druglist-item_substituteMedicineDetails__0wLPq">
                    <div
                      style={{
                        display: "flex",
                        justifyContent: "space-between",
                        flexDirection: "row",
                      }}
                    >
                      <div className="druglist-item_contentContainer__OyI1B">
                        <div
                          className="druglist-item_imgContainer__3MOmx"
                          style={{
                            display: "flex",
                            alignItems: "flex-end",
                            flexWrap: "nowrap",
                          }}
                        >
                          <div
                            className="druglist-item_itemImg__KBvIo"
                            style={{
                              backgroundImage:
                                "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                            }}
                          ></div>
                        </div>
                        <div
                          className="druglist-item_itemDetails__JhI3f"
                          style={{ marginTop: "8px" }}
                        >
                          <div className="druglist-item_itemName__YnmLq">
                            Pyrakem IV 1000mg Infusion 100ml
                          </div>
                          <div className="druglist-item_substituteManufacturerName__fECQK">
                            Alkem Laboratories Ltd
                          </div>
                          <div className="druglist-item_Quantity__y1rmm">
                            Vial of 100 ml
                          </div>
                          <div
                            className="druglist-item_addtoCartContainer__0XkUC"
                            style={{
                              display: "flex",
                              justifyContent: "space-between",
                              alignItems: "flex-end",
                            }}
                          >
                            <div className="druglist-item_substitutePriceContainer__75Y7h">
                              <div className="druglist-item_substitutePrice__Qdsht">
                                <p className="druglist-item_discountedPrice__ftOvo">
                                  ₹138
                                </p>
                              </div>
                              <p className="druglist-item_itemMrp__aSi9J">
                                MRP ₹<span>276</span>
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          justifyContent: "flex-end",
                          alignItems: "flex-end",
                          position: "relative",
                        }}
                      >
                        <div className="druglist-item_SavingLabel__B_Smj">
                          49% Savings
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </a>
          <div className="product-listing_divider__aiMpd"></div>
          <a href="/product-display/dolo-mf-oral-suspension/1012435">
            <div className="druglist-item_itemContainer__Uqt1t">
              <div className="druglist-item_itemHeading__cWGyl">
                <div className="druglist-item_saltMobileContainer__0R4yP">
                  <div className="druglist-item_saltMobileContainer__content__ILi6H">
                    <div className="druglist-item_saltMobileContainer__title__3zs9s">
                      <img
                        alt="Molecule"
                        loading="lazy"
                        width="32"
                        height="32"
                        decoding="async"
                        className="druglist-item_saltMobileContainer__icon__L13mN"
                        srcSet={`
                            ${saltImage}?w=32&amp;q=80 1x,
                            ${saltImage}?w=64&amp;q=80 2x
                          `}
                        src={`${saltImage}?w=64&amp;q=80`}
                        style={{ color: "transparent", display: "none" }}
                      />
                      <div className="druglist-item_saltMobileContainer__titleText__9UmPF">
                        Salt Composition in both
                      </div>
                    </div>
                    <div className="druglist-item_saltMobileContainer__composition__EsLOi">
                      <p className="druglist-item_saltMobileContainer__composition__less_salt__B_Gip">
                        Mefenamic Acid 100mg + Paracetamol (Acetaminophen) 250mg
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="druglist-item_fullDetailsContainer__RCxNm">
                <div className="druglist-item_genericMedicineDetails__rdmT5">
                  <div
                    style={{
                      display: "flex",
                      justifyContent: "space-between",
                      flexDirection: "row",
                    }}
                  >
                    <div className="druglist-item_contentContainer__OyI1B">
                      <div
                        className="druglist-item_imgContainer__3MOmx"
                        style={{
                          display: "flex",
                          alignItems: "flex-end",
                          flexWrap: "nowrap",
                        }}
                      >
                        <div
                          className="druglist-item_itemImg__KBvIo"
                          style={{
                            backgroundImage:
                              "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                          }}
                        ></div>
                      </div>
                      <div
                        className="druglist-item_itemDetails__JhI3f"
                        style={{ marginTop: "8px" }}
                      >
                        <div className="druglist-item_itemName__YnmLq">
                          Dolo-MF Oral Suspension
                        </div>
                        <div className="druglist-item_genericManufacturerName__Du3NS">
                          Micro Labs Ltd
                        </div>
                        <div className="druglist-item_Quantity__y1rmm">
                          Bottle of 60 ml
                        </div>
                        <div className="druglist-item_itemMrp__aSi9J">MRP</div>
                        <p className="druglist-item_genericPrice__0u9Ol">₹63</p>
                      </div>
                    </div>
                  </div>
                </div>
                <div
                  className="druglist-item_substituteHeaderContainer__ezxIX"
                  style={{ width: "65%" }}
                >
                  <div className="druglist-item_substituteMedicineDetails__0wLPq">
                    <div
                      style={{
                        display: "flex",
                        justifyContent: "space-between",
                        flexDirection: "row",
                      }}
                    >
                      <div className="druglist-item_contentContainer__OyI1B">
                        <div
                          className="druglist-item_imgContainer__3MOmx"
                          style={{
                            display: "flex",
                            alignItems: "flex-end",
                            flexWrap: "nowrap",
                          }}
                        >
                          <div
                            className="druglist-item_itemImg__KBvIo"
                            style={{
                              backgroundImage:
                                "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                            }}
                          ></div>
                        </div>
                        <div
                          className="druglist-item_itemDetails__JhI3f"
                          style={{ marginTop: "8px" }}
                        >
                          <div className="druglist-item_itemName__YnmLq">
                            Mifer Plus DS 100/250mg Oral Suspension 60ml
                          </div>
                          <div className="druglist-item_substituteManufacturerName__fECQK">
                            Glenmark Pharmaceuticals Ltd
                          </div>
                          <div className="druglist-item_Quantity__y1rmm">
                            Bottle of 60 ml
                          </div>
                          <div
                            className="druglist-item_addtoCartContainer__0XkUC"
                            style={{
                              display: "flex",
                              justifyContent: "space-between",
                              alignItems: "flex-end",
                            }}
                          >
                            <div className="druglist-item_substitutePriceContainer__75Y7h">
                              <div className="druglist-item_substitutePrice__Qdsht">
                                <p className="druglist-item_discountedPrice__ftOvo">
                                  ₹43
                                </p>
                              </div>
                              <p className="druglist-item_itemMrp__aSi9J">
                                MRP ₹<span>86</span>
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          justifyContent: "flex-end",
                          alignItems: "flex-end",
                          position: "relative",
                        }}
                      >
                        <div className="druglist-item_SavingLabel__B_Smj">
                          32% Savings
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </a>
          <div className="product-listing_divider__aiMpd"></div>
          <a href="/product-display/dolo-120-suspension/1200869">
            <div className="druglist-item_itemContainer__Uqt1t">
              <div className="druglist-item_itemHeading__cWGyl">
                <div className="druglist-item_saltMobileContainer__0R4yP">
                  <div className="druglist-item_saltMobileContainer__content__ILi6H">
                    <div className="druglist-item_saltMobileContainer__title__3zs9s">
                      <img
                        alt="Molecule"
                        loading="lazy"
                        width="32"
                        height="32"
                        decoding="async"
                        className="druglist-item_saltMobileContainer__icon__L13mN"
                        srcSet={`
                            ${saltImage}?w=32&amp;q=80 1x,
                            ${saltImage}?w=64&amp;q=80 2x
                          `}
                        src={`${saltImage}?w=64&amp;q=80`}
                        style={{ color: "transparent", display: "none" }}
                      />
                      <div className="druglist-item_saltMobileContainer__titleText__9UmPF">
                        Salt Composition in both
                      </div>
                    </div>
                    <div className="druglist-item_saltMobileContainer__composition__EsLOi">
                      <p className="druglist-item_saltMobileContainer__composition__less_salt__B_Gip">
                        Paracetamol (Acetaminophen) 120mg
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="druglist-item_fullDetailsContainer__RCxNm">
                <div className="druglist-item_genericMedicineDetails__rdmT5">
                  <div
                    style={{
                      display: "flex",
                      justifyContent: "space-between",
                      flexDirection: "row",
                    }}
                  >
                    <div className="druglist-item_contentContainer__OyI1B">
                      <div
                        className="druglist-item_imgContainer__3MOmx"
                        style={{
                          display: "flex",
                          alignItems: "flex-end",
                          flexWrap: "nowrap",
                        }}
                      >
                        <div
                          className="druglist-item_itemImg__KBvIo"
                          style={{
                            backgroundImage:
                              "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                          }}
                        ></div>
                      </div>
                      <div
                        className="druglist-item_itemDetails__JhI3f"
                        style={{ marginTop: "8px" }}
                      >
                        <div className="druglist-item_itemName__YnmLq">
                          Dolo 120 Suspension
                        </div>
                        <div className="druglist-item_genericManufacturerName__Du3NS">
                          Micro Labs Ltd
                        </div>
                        <div className="druglist-item_Quantity__y1rmm">
                          Bottle of 60 ml
                        </div>
                        <div className="druglist-item_itemMrp__aSi9J">MRP</div>
                        <p className="druglist-item_genericPrice__0u9Ol">₹41</p>
                      </div>
                    </div>
                  </div>
                </div>
                <div
                  className="druglist-item_substituteHeaderContainer__ezxIX"
                  style={{ width: "65%" }}
                >
                  <div className="druglist-item_substituteMedicineDetails__0wLPq">
                    <div
                      style={{
                        display: "flex",
                        justifyContent: "space-between",
                        flexDirection: "row",
                      }}
                    >
                      <div className="druglist-item_contentContainer__OyI1B">
                        <div
                          className="druglist-item_imgContainer__3MOmx"
                          style={{
                            display: "flex",
                            alignItems: "flex-end",
                            flexWrap: "nowrap",
                          }}
                        >
                          <div
                            className="druglist-item_itemImg__KBvIo"
                            style={{
                              backgroundImage:
                                "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                            }}
                          ></div>
                        </div>
                        <div
                          className="druglist-item_itemDetails__JhI3f"
                          style={{ marginTop: "8px" }}
                        >
                          <div className="druglist-item_itemName__YnmLq">
                            Aristomol 120mg Suspension 60ml
                          </div>
                          <div className="druglist-item_substituteManufacturerName__fECQK">
                            Aristo Pharmaceuticals Pvt Ltd
                          </div>
                          <div className="druglist-item_Quantity__y1rmm">
                            Bottle of 60 ml
                          </div>
                          <div
                            className="druglist-item_addtoCartContainer__0XkUC"
                            style={{
                              display: "flex",
                              justifyContent: "space-between",
                              alignItems: "flex-end",
                            }}
                          >
                            <div className="druglist-item_substitutePriceContainer__75Y7h">
                              <div className="druglist-item_substitutePrice__Qdsht">
                                <p className="druglist-item_discountedPrice__ftOvo">
                                  ₹34.8
                                </p>
                              </div>
                              <p className="druglist-item_itemMrp__aSi9J">
                                MRP ₹<span>41</span>
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          justifyContent: "flex-end",
                          alignItems: "flex-end",
                          position: "relative",
                        }}
                      >
                        <div className="druglist-item_SavingLabel__B_Smj">
                          15% Savings
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </a>
          <div className="product-listing_divider__aiMpd"></div>
          <a href="/product-display/dolo-1000mg-tablet/1100599">
            <div className="druglist-item_itemContainer__Uqt1t">
              <div className="druglist-item_itemHeading__cWGyl">
                <div className="druglist-item_saltMobileContainer__0R4yP">
                  <div className="druglist-item_saltMobileContainer__content__ILi6H">
                    <div className="druglist-item_saltMobileContainer__title__3zs9s">
                      <img
                        alt="Molecule"
                        loading="lazy"
                        width="32"
                        height="32"
                        decoding="async"
                        className="druglist-item_saltMobileContainer__icon__L13mN"
                        srcSet={`
                            ${saltImage}?w=32&amp;q=80 1x,
                            ${saltImage}?w=64&amp;q=80 2x
                          `}
                        src={`${saltImage}?w=64&amp;q=80`}
                        style={{ color: "transparent", display: "none" }}
                      />
                      <div className="druglist-item_saltMobileContainer__titleText__9UmPF">
                        Salt Composition in both
                      </div>
                    </div>
                    <div className="druglist-item_saltMobileContainer__composition__EsLOi">
                      <p className="druglist-item_saltMobileContainer__composition__less_salt__B_Gip">
                        Paracetamol (Acetaminophen) 1000mg
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="druglist-item_fullDetailsContainer__RCxNm">
                <div className="druglist-item_genericMedicineDetails__rdmT5">
                  <div
                    style={{
                      display: "flex",
                      justifyContent: "space-between",
                      flexDirection: "row",
                    }}
                  >
                    <div className="druglist-item_contentContainer__OyI1B">
                      <div
                        className="druglist-item_imgContainer__3MOmx"
                        style={{
                          display: "flex",
                          alignItems: "flex-end",
                          flexWrap: "nowrap",
                        }}
                      >
                        <div
                          className="druglist-item_itemImg__KBvIo"
                          style={{
                            backgroundImage:
                              "url('https://platinumrxassets.blob.core.windows.net/platinumrx-assets/default_image.jpg')",
                          }}
                        ></div>
                      </div>
                      <div
                        className="druglist-item_itemDetails__JhI3f"
                        style={{ marginTop: "8px" }}
                      >
                        <div className="druglist-item_itemName__YnmLq">
                          Dolo 1000mg Tablet
                        </div>
                        <div className="druglist-item_genericManufacturerName__Du3NS">
                          Micro Labs Ltd
                        </div>
                        <div className="druglist-item_Quantity__y1rmm">
                          Strip of 10 tablets
                        </div>
                        <div className="druglist-item_itemMrp__aSi9J">MRP</div>
                        <p className="druglist-item_genericPrice__0u9Ol">
                          ₹45.6
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
                <div
                  className="druglist-item_substituteHeaderContainer__ezxIX"
                  style={{ width: "65%" }}
                >
                  <div className="druglist-item_substituteMedicineDetails__0wLPq">
                    <div
                      style={{
                        display: "flex",
                        justifyContent: "space-between",
                        flexDirection: "row",
                      }}
                    >
                      <div className="druglist-item_contentContainer__OyI1B">
                        <div
                          className="druglist-item_imgContainer__3MOmx"
                          style={{
                            display: "flex",
                            alignItems: "flex-end",
                            flexWrap: "nowrap",
                          }}
                        >
                          <div
                            className="druglist-item_itemImg__KBvIo"
                            style={{
                              backgroundImage:
                                "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                            }}
                          ></div>
                        </div>
                        <div
                          className="druglist-item_itemDetails__JhI3f"
                          style={{ marginTop: "8px" }}
                        >
                          <div className="druglist-item_itemName__YnmLq">
                            Pyrigesic 1000mg Tab 10s
                          </div>
                          <div className="druglist-item_substituteManufacturerName__fECQK">
                            East India Pharmaceutical Works Ltd
                          </div>
                          <div className="druglist-item_Quantity__y1rmm">
                            Strip of 10 tablets
                          </div>
                          <div
                            className="druglist-item_addtoCartContainer__0XkUC"
                            style={{
                              display: "flex",
                              justifyContent: "space-between",
                              alignItems: "flex-end",
                            }}
                          >
                            <div className="druglist-item_substitutePriceContainer__75Y7h">
                              <div className="druglist-item_substitutePrice__Qdsht">
                                <p className="druglist-item_discountedPrice__ftOvo">
                                  ₹32.8
                                </p>
                              </div>
                              <p className="druglist-item_itemMrp__aSi9J">
                                MRP ₹<span>40</span>
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          justifyContent: "flex-end",
                          alignItems: "flex-end",
                          position: "relative",
                        }}
                      >
                        <div className="druglist-item_SavingLabel__B_Smj">
                          28% Savings
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </a>
          <div className="product-listing_divider__aiMpd"></div>
          <a href="/product-display/dolo-drops/1109844">
            <div className="druglist-item_itemContainer__Uqt1t">
              <div className="druglist-item_itemHeading__cWGyl">
                <div className="druglist-item_saltMobileContainer__0R4yP">
                  <div className="druglist-item_saltMobileContainer__content__ILi6H">
                    <div className="druglist-item_saltMobileContainer__title__3zs9s">
                      <img
                        alt="Molecule"
                        loading="lazy"
                        width="32"
                        height="32"
                        decoding="async"
                        className="druglist-item_saltMobileContainer__icon__L13mN"
                        srcSet={`
                            ${saltImage}?w=32&amp;q=80 1x,
                            ${saltImage}?w=64&amp;q=80 2x
                          `}
                        src={`${saltImage}?w=64&amp;q=80`}
                        style={{ color: "transparent", display: "none" }}
                      />
                      <div className="druglist-item_saltMobileContainer__titleText__9UmPF">
                        Salt Composition in both
                      </div>
                    </div>
                    <div className="druglist-item_saltMobileContainer__composition__EsLOi">
                      <p className="druglist-item_saltMobileContainer__composition__less_salt__B_Gip">
                        Paracetamol (Acetaminophen) 100mg
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="druglist-item_fullDetailsContainer__RCxNm">
                <div className="druglist-item_genericMedicineDetails__rdmT5">
                  <div
                    style={{
                      display: "flex",
                      justifyContent: "space-between",
                      flexDirection: "row",
                    }}
                  >
                    <div className="druglist-item_contentContainer__OyI1B">
                      <div
                        className="druglist-item_imgContainer__3MOmx"
                        style={{
                          display: "flex",
                          alignItems: "flex-end",
                          flexWrap: "nowrap",
                        }}
                      >
                        <div
                          className="druglist-item_itemImg__KBvIo"
                          style={{
                            backgroundImage:
                              "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                          }}
                        ></div>
                      </div>
                      <div
                        className="druglist-item_itemDetails__JhI3f"
                        style={{ marginTop: "8px" }}
                      >
                        <div className="druglist-item_itemName__YnmLq">
                          Dolo Drops
                        </div>
                        <div className="druglist-item_genericManufacturerName__Du3NS">
                          Micro Labs Ltd
                        </div>
                        <div className="druglist-item_Quantity__y1rmm">
                          Packet of 15 ml
                        </div>
                        <div className="druglist-item_itemMrp__aSi9J">MRP</div>
                        <p className="druglist-item_genericPrice__0u9Ol">
                          ₹30.1
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
                <div
                  className="druglist-item_substituteHeaderContainer__ezxIX"
                  style={{ width: "65%" }}
                >
                  <div className="druglist-item_substituteMedicineDetails__0wLPq">
                    <div
                      style={{
                        display: "flex",
                        justifyContent: "space-between",
                        flexDirection: "row",
                      }}
                    >
                      <div className="druglist-item_contentContainer__OyI1B">
                        <div
                          className="druglist-item_imgContainer__3MOmx"
                          style={{
                            display: "flex",
                            alignItems: "flex-end",
                            flexWrap: "nowrap",
                          }}
                        >
                          <div
                            className="druglist-item_itemImg__KBvIo"
                            style={{
                              backgroundImage:
                                "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                            }}
                          ></div>
                        </div>
                        <div
                          className="druglist-item_itemDetails__JhI3f"
                          style={{ marginTop: "8px" }}
                        >
                          <div className="druglist-item_itemName__YnmLq">
                            Leemol 100mg Oral Suspension 15ml
                          </div>
                          <div className="druglist-item_substituteManufacturerName__fECQK">
                            Leeford Healthcare Ltd
                          </div>
                          <div className="druglist-item_Quantity__y1rmm">
                            Packet of 15 ml
                          </div>
                          <div
                            className="druglist-item_addtoCartContainer__0XkUC"
                            style={{
                              display: "flex",
                              justifyContent: "space-between",
                              alignItems: "flex-end",
                            }}
                          >
                            <div className="druglist-item_substitutePriceContainer__75Y7h">
                              <div className="druglist-item_substitutePrice__Qdsht">
                                <p className="druglist-item_discountedPrice__ftOvo">
                                  ₹22.6
                                </p>
                              </div>
                              <p className="druglist-item_itemMrp__aSi9J">
                                MRP ₹<span>29</span>
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          justifyContent: "flex-end",
                          alignItems: "flex-end",
                          position: "relative",
                        }}
                      >
                        <div className="druglist-item_SavingLabel__B_Smj">
                          25% Savings
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </a>
          <div className="product-listing_divider__aiMpd"></div>
          <a href="/product-display/dolo-500-tablet-15-tablet/1207615">
            <div className="druglist-item_itemContainer__Uqt1t">
              <div className="druglist-item_itemHeading__cWGyl">
                <div className="druglist-item_saltMobileContainer__0R4yP">
                  <div className="druglist-item_saltMobileContainer__content__ILi6H">
                    <div className="druglist-item_saltMobileContainer__title__3zs9s">
                      <img
                        alt="Molecule"
                        loading="lazy"
                        width="32"
                        height="32"
                        decoding="async"
                        className="druglist-item_saltMobileContainer__icon__L13mN"
                        srcSet={`
                            ${saltImage}?w=32&amp;q=80 1x,
                            ${saltImage}?w=64&amp;q=80 2x
                          `}
                        src={`${saltImage}?w=64&amp;q=80`}
                        style={{ color: "transparent", display: "none" }}
                      />
                      <div className="druglist-item_saltMobileContainer__titleText__9UmPF">
                        Salt Composition in both
                      </div>
                    </div>
                    <div className="druglist-item_saltMobileContainer__composition__EsLOi">
                      <p className="druglist-item_saltMobileContainer__composition__less_salt__B_Gip">
                        Paracetamol (Acetaminophen) 500mg
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="druglist-item_fullDetailsContainer__RCxNm">
                <div className="druglist-item_genericMedicineDetails__rdmT5">
                  <div
                    style={{
                      display: "flex",
                      justifyContent: "space-between",
                      flexDirection: "row",
                    }}
                  >
                    <div className="druglist-item_contentContainer__OyI1B">
                      <div
                        className="druglist-item_imgContainer__3MOmx"
                        style={{
                          display: "flex",
                          alignItems: "flex-end",
                          flexWrap: "nowrap",
                        }}
                      >
                        <div
                          className="druglist-item_itemImg__KBvIo"
                          style={{
                            backgroundImage:
                              "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                          }}
                        ></div>
                      </div>
                      <div
                        className="druglist-item_itemDetails__JhI3f"
                        style={{ marginTop: "8px" }}
                      >
                        <div className="druglist-item_itemName__YnmLq">
                          Dolo 500 Tablet 15 Tablet
                        </div>
                        <div className="druglist-item_genericManufacturerName__Du3NS">
                          Micro Labs Ltd
                        </div>
                        <div className="druglist-item_Quantity__y1rmm">
                          Strip of 15 tablets
                        </div>
                        <div className="druglist-item_itemMrp__aSi9J">MRP</div>
                        <p className="druglist-item_genericPrice__0u9Ol">
                          ₹15.1
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
                <div
                  className="druglist-item_substituteHeaderContainer__ezxIX"
                  style={{ width: "65%" }}
                >
                  <div className="druglist-item_substituteMedicineDetails__0wLPq">
                    <div
                      style={{
                        display: "flex",
                        justifyContent: "space-between",
                        flexDirection: "row",
                      }}
                    >
                      <div className="druglist-item_contentContainer__OyI1B">
                        <div
                          className="druglist-item_imgContainer__3MOmx"
                          style={{
                            display: "flex",
                            alignItems: "flex-end",
                            flexWrap: "nowrap",
                          }}
                        >
                          <div
                            className="druglist-item_itemImg__KBvIo"
                            style={{
                              backgroundImage:
                                "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                            }}
                          ></div>
                        </div>
                        <div
                          className="druglist-item_itemDetails__JhI3f"
                          style={{ marginTop: "8px" }}
                        >
                          <div className="druglist-item_itemName__YnmLq">
                            Paracip 500mg Tab 10s
                          </div>
                          <div className="druglist-item_substituteManufacturerName__fECQK">
                            Cipla Ltd
                          </div>
                          <div className="druglist-item_Quantity__y1rmm">
                            Strip of 10 tablets
                          </div>
                          <div
                            className="druglist-item_addtoCartContainer__0XkUC"
                            style={{
                              display: "flex",
                              justifyContent: "space-between",
                              alignItems: "flex-end",
                            }}
                          >
                            <div className="druglist-item_substitutePriceContainer__75Y7h">
                              <div className="druglist-item_substitutePrice__Qdsht">
                                <p className="druglist-item_discountedPrice__ftOvo">
                                  ₹7.9
                                </p>
                              </div>
                              <p className="druglist-item_itemMrp__aSi9J">
                                MRP ₹<span>10.1</span>
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          justifyContent: "flex-end",
                          alignItems: "flex-end",
                          position: "relative",
                        }}
                      >
                        <div className="druglist-item_SavingLabel__B_Smj">
                          22% Savings
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </a>
          <div className="product-listing_divider__aiMpd"></div>
          <a href="/product-display/dolo-th-4-tablet/1184407">
            <div className="druglist-item_itemContainer__Uqt1t">
              <div className="druglist-item_itemHeading__cWGyl">
                <div className="druglist-item_saltMobileContainer__0R4yP">
                  <div className="druglist-item_saltMobileContainer__content__ILi6H">
                    <div className="druglist-item_saltMobileContainer__title__3zs9s">
                      <img
                        alt="Molecule"
                        loading="lazy"
                        width="32"
                        height="32"
                        decoding="async"
                        className="druglist-item_saltMobileContainer__icon__L13mN"
                        srcSet={`
                            ${saltImage}?w=32&amp;q=80 1x,
                            ${saltImage}?w=64&amp;q=80 2x
                          `}
                        src={`${saltImage}?w=64&amp;q=80`}
                        style={{ color: "transparent", display: "none" }}
                      />
                      <div className="druglist-item_saltMobileContainer__titleText__9UmPF">
                        Salt Composition in both
                      </div>
                    </div>
                    <div className="druglist-item_saltMobileContainer__composition__EsLOi">
                      <p className="druglist-item_saltMobileContainer__composition__less_salt__B_Gip">
                        Paracetamol (Acetaminophen) 500mg + Thiocolchicoside 4mg
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="druglist-item_fullDetailsContainer__RCxNm">
                <div className="druglist-item_genericMedicineDetails__rdmT5">
                  <div
                    style={{
                      display: "flex",
                      justifyContent: "space-between",
                      flexDirection: "row",
                    }}
                  >
                    <div className="druglist-item_contentContainer__OyI1B">
                      <div
                        className="druglist-item_imgContainer__3MOmx"
                        style={{
                          display: "flex",
                          alignItems: "flex-end",
                          flexWrap: "nowrap",
                        }}
                      >
                        <div
                          className="druglist-item_itemImg__KBvIo"
                          style={{
                            backgroundImage:
                              "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                          }}
                        ></div>
                      </div>
                      <div
                        className="druglist-item_itemDetails__JhI3f"
                        style={{ marginTop: "8px" }}
                      >
                        <div className="druglist-item_itemName__YnmLq">
                          Dolo TH 4 Tablet
                        </div>
                        <div className="druglist-item_genericManufacturerName__Du3NS">
                          Micro Labs Ltd
                        </div>
                        <div className="druglist-item_Quantity__y1rmm">
                          Strip of 10 tablets
                        </div>
                        <div className="druglist-item_itemMrp__aSi9J">MRP</div>
                        <p className="druglist-item_genericPrice__0u9Ol">
                          ₹189.7
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
                <div
                  className="druglist-item_substituteHeaderContainer__ezxIX"
                  style={{ width: "65%" }}
                >
                  <div className="druglist-item_substituteMedicineDetails__0wLPq">
                    <div
                      style={{
                        display: "flex",
                        justifyContent: "space-between",
                        flexDirection: "row",
                      }}
                    >
                      <div className="druglist-item_contentContainer__OyI1B">
                        <div
                          className="druglist-item_imgContainer__3MOmx"
                          style={{
                            display: "flex",
                            alignItems: "flex-end",
                            flexWrap: "nowrap",
                          }}
                        >
                          <div
                            className="druglist-item_itemImg__KBvIo"
                            style={{
                              backgroundImage:
                                "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                            }}
                          ></div>
                        </div>
                        <div
                          className="druglist-item_itemDetails__JhI3f"
                          style={{ marginTop: "8px" }}
                        >
                          <div className="druglist-item_itemName__YnmLq">
                            Xykaa MR 500/4mg Tab 10s
                          </div>
                          <div className="druglist-item_substituteManufacturerName__fECQK">
                            Troikaa Pharmaceuticals Ltd
                          </div>
                          <div className="druglist-item_Quantity__y1rmm">
                            Strip of 10 tablets
                          </div>
                          <div
                            className="druglist-item_addtoCartContainer__0XkUC"
                            style={{
                              display: "flex",
                              justifyContent: "space-between",
                              alignItems: "flex-end",
                            }}
                          >
                            <div className="druglist-item_substitutePriceContainer__75Y7h">
                              <div className="druglist-item_substitutePrice__Qdsht">
                                <p className="druglist-item_discountedPrice__ftOvo">
                                  ₹205.6
                                </p>
                              </div>
                              <p className="druglist-item_itemMrp__aSi9J">
                                MRP ₹<span>241.9</span>
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          justifyContent: "flex-end",
                          alignItems: "flex-end",
                          position: "relative",
                        }}
                      >
                        <div className="druglist-item_SavingLabel__B_Smj">
                          Top Brand
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </a>
          <div className="product-listing_divider__aiMpd"></div>
          <a href="/product-display/dolox-20mg-tablet-dt/1044691">
            <div className="druglist-item_itemContainer__Uqt1t">
              <div className="druglist-item_itemHeading__cWGyl">
                <div className="druglist-item_saltMobileContainer__0R4yP">
                  <div className="druglist-item_saltMobileContainer__content__ILi6H">
                    <div className="druglist-item_saltMobileContainer__title__3zs9s">
                      <img
                        alt="Molecule"
                        loading="lazy"
                        width="32"
                        height="32"
                        decoding="async"
                        className="druglist-item_saltMobileContainer__icon__L13mN"
                        srcSet={`
                            ${saltImage}?w=32&amp;q=80 1x,
                            ${saltImage}?w=64&amp;q=80 2x
                          `}
                        src={`${saltImage}?w=64&amp;q=80`}
                        style={{ color: "transparent", display: "none" }}
                      />
                      <div className="druglist-item_saltMobileContainer__titleText__9UmPF">
                        Salt Composition in both
                      </div>
                    </div>
                    <div className="druglist-item_saltMobileContainer__composition__EsLOi">
                      <p className="druglist-item_saltMobileContainer__composition__less_salt__B_Gip">
                        Piroxicam 20mg
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="druglist-item_fullDetailsContainer__RCxNm">
                <div className="druglist-item_genericMedicineDetails__rdmT5">
                  <div
                    style={{
                      display: "flex",
                      justifyContent: "space-between",
                      flexDirection: "row",
                    }}
                  >
                    <div className="druglist-item_contentContainer__OyI1B">
                      <div
                        className="druglist-item_imgContainer__3MOmx"
                        style={{
                          display: "flex",
                          alignItems: "flex-end",
                          flexWrap: "nowrap",
                        }}
                      >
                        <div
                          className="druglist-item_itemImg__KBvIo"
                          style={{
                            backgroundImage:
                              "url('https://platinumrxassets.blob.core.windows.net/platinumrx-assets/default_image.jpg')",
                          }}
                        ></div>
                      </div>
                      <div
                        className="druglist-item_itemDetails__JhI3f"
                        style={{ marginTop: "8px" }}
                      >
                        <div className="druglist-item_itemName__YnmLq">
                          Dolox 20mg Tablet DT
                        </div>
                        <div className="druglist-item_genericManufacturerName__Du3NS">
                          Aronex Life Sciences Pvt Ltd
                        </div>
                        <div className="druglist-item_Quantity__y1rmm">
                          Strip of 10 tablets
                        </div>
                        <div className="druglist-item_itemMrp__aSi9J">MRP</div>
                        <p className="druglist-item_genericPrice__0u9Ol">₹39</p>
                      </div>
                    </div>
                  </div>
                </div>
                <div
                  className="druglist-item_substituteHeaderContainer__ezxIX"
                  style={{ width: "65%" }}
                >
                  <div className="druglist-item_substituteMedicineDetails__0wLPq">
                    <div
                      style={{
                        display: "flex",
                        justifyContent: "space-between",
                        flexDirection: "row",
                      }}
                    >
                      <div className="druglist-item_contentContainer__OyI1B">
                        <div
                          className="druglist-item_imgContainer__3MOmx"
                          style={{
                            display: "flex",
                            alignItems: "flex-end",
                            flexWrap: "nowrap",
                          }}
                        >
                          <div
                            className="druglist-item_itemImg__KBvIo"
                            style={{
                              backgroundImage:
                                "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                            }}
                          ></div>
                        </div>
                        <div
                          className="druglist-item_itemDetails__JhI3f"
                          style={{ marginTop: "8px" }}
                        >
                          <div className="druglist-item_itemName__YnmLq">
                            Pirox DT 20mg Tab DT 10s
                          </div>
                          <div className="druglist-item_substituteManufacturerName__fECQK">
                            Cipla Ltd
                          </div>
                          <div className="druglist-item_Quantity__y1rmm">
                            Strip of 10 tablets
                          </div>
                          <div
                            className="druglist-item_addtoCartContainer__0XkUC"
                            style={{
                              display: "flex",
                              justifyContent: "space-between",
                              alignItems: "flex-end",
                            }}
                          >
                            <div className="druglist-item_substitutePriceContainer__75Y7h">
                              <div className="druglist-item_substitutePrice__Qdsht">
                                <p className="druglist-item_discountedPrice__ftOvo">
                                  ₹47.3
                                </p>
                              </div>
                              <p className="druglist-item_itemMrp__aSi9J">
                                MRP ₹<span>94.5</span>
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          justifyContent: "flex-end",
                          alignItems: "flex-end",
                          position: "relative",
                        }}
                      >
                        <div className="druglist-item_SavingLabel__B_Smj">
                          Top Brand
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </a>
          <div className="product-listing_divider__aiMpd"></div>
          <a href="/product-display/dolox-40mg-injection/1063144">
            <div className="druglist-item_itemContainer__Uqt1t">
              <div className="druglist-item_itemHeading__cWGyl">
                <div className="druglist-item_saltMobileContainer__0R4yP">
                  <div className="druglist-item_saltMobileContainer__content__ILi6H">
                    <div className="druglist-item_saltMobileContainer__title__3zs9s">
                      <img
                        alt="Molecule"
                        loading="lazy"
                        width="32"
                        height="32"
                        decoding="async"
                        className="druglist-item_saltMobileContainer__icon__L13mN"
                        srcSet={`
                            ${saltImage}?w=32&amp;q=80 1x,
                            ${saltImage}?w=64&amp;q=80 2x
                          `}
                        src={`${saltImage}?w=64&amp;q=80`}
                        style={{ color: "transparent", display: "none" }}
                      />
                      <div className="druglist-item_saltMobileContainer__titleText__9UmPF">
                        Salt Composition in both
                      </div>
                    </div>
                    <div className="druglist-item_saltMobileContainer__composition__EsLOi">
                      <p className="druglist-item_saltMobileContainer__composition__less_salt__B_Gip">
                        Piroxicam 40mg
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="druglist-item_fullDetailsContainer__RCxNm">
                <div className="druglist-item_genericMedicineDetails__rdmT5">
                  <div
                    style={{
                      display: "flex",
                      justifyContent: "space-between",
                      flexDirection: "row",
                    }}
                  >
                    <div className="druglist-item_contentContainer__OyI1B">
                      <div
                        className="druglist-item_imgContainer__3MOmx"
                        style={{
                          display: "flex",
                          alignItems: "flex-end",
                          flexWrap: "nowrap",
                        }}
                      >
                        <div
                          className="druglist-item_itemImg__KBvIo"
                          style={{
                            backgroundImage:
                              "url('https://platinumrxassets.blob.core.windows.net/platinumrx-assets/default_image.jpg')",
                          }}
                        ></div>
                      </div>
                      <div
                        className="druglist-item_itemDetails__JhI3f"
                        style={{ marginTop: "8px" }}
                      >
                        <div className="druglist-item_itemName__YnmLq">
                          Dolox 40mg Injection
                        </div>
                        <div className="druglist-item_genericManufacturerName__Du3NS">
                          Aronex Life Sciences Pvt Ltd
                        </div>
                        <div className="druglist-item_Quantity__y1rmm">
                          Vial of 2 ml
                        </div>
                        <div className="druglist-item_itemMrp__aSi9J">MRP</div>
                        <p className="druglist-item_genericPrice__0u9Ol">₹38</p>
                      </div>
                    </div>
                  </div>
                </div>
                <div
                  className="druglist-item_substituteHeaderContainer__ezxIX"
                  style={{ width: "65%" }}
                >
                  <div className="druglist-item_substituteMedicineDetails__0wLPq">
                    <div
                      style={{
                        display: "flex",
                        justifyContent: "space-between",
                        flexDirection: "row",
                      }}
                    >
                      <div className="druglist-item_contentContainer__OyI1B">
                        <div
                          className="druglist-item_imgContainer__3MOmx"
                          style={{
                            display: "flex",
                            alignItems: "flex-end",
                            flexWrap: "nowrap",
                          }}
                        >
                          <div
                            className="druglist-item_itemImg__KBvIo"
                            style={{
                              backgroundImage:
                                "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                            }}
                          ></div>
                        </div>
                        <div
                          className="druglist-item_itemDetails__JhI3f"
                          style={{ marginTop: "8px" }}
                        >
                          <div className="druglist-item_itemName__YnmLq">
                            Mizocam 40mg Inj 2ml
                          </div>
                          <div className="druglist-item_substituteManufacturerName__fECQK">
                            Leeford Healthcare Ltd
                          </div>
                          <div className="druglist-item_Quantity__y1rmm">
                            Vial of 2 ml
                          </div>
                          <div
                            className="druglist-item_addtoCartContainer__0XkUC"
                            style={{
                              display: "flex",
                              justifyContent: "space-between",
                              alignItems: "flex-end",
                            }}
                          >
                            <div className="druglist-item_substitutePriceContainer__75Y7h">
                              <div className="druglist-item_substitutePrice__Qdsht">
                                <p className="druglist-item_discountedPrice__ftOvo">
                                  ₹11
                                </p>
                              </div>
                              <p className="druglist-item_itemMrp__aSi9J">
                                MRP ₹<span>22</span>
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          justifyContent: "flex-end",
                          alignItems: "flex-end",
                          position: "relative",
                        }}
                      >
                        <div className="druglist-item_SavingLabel__B_Smj">
                          71% Savings
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </a>
          <div className="product-listing_divider__aiMpd"></div>
          <a href="/product-display/doloneuron-nt-200-10mg-tab-10s/1302582">
            <div className="druglist-item_itemContainer__Uqt1t">
              <div className="druglist-item_itemHeading__cWGyl">
                <div className="druglist-item_saltMobileContainer__0R4yP">
                  <div className="druglist-item_saltMobileContainer__content__ILi6H">
                    <div className="druglist-item_saltMobileContainer__title__3zs9s">
                      <img
                        alt="Molecule"
                        loading="lazy"
                        width="32"
                        height="32"
                        decoding="async"
                        className="druglist-item_saltMobileContainer__icon__L13mN"
                        srcSet={`
                            ${saltImage}?w=32&amp;q=80 1x,
                            ${saltImage}?w=64&amp;q=80 2x
                          `}
                        src={`${saltImage}?w=64&amp;q=80`}
                        style={{ color: "transparent", display: "none" }}
                      />
                      <div className="druglist-item_saltMobileContainer__titleText__9UmPF">
                        Salt Composition
                      </div>
                    </div>
                    <div className="druglist-item_saltMobileContainer__composition__EsLOi">
                      <p className="druglist-item_saltMobileContainer__composition__less_salt__B_Gip">
                        Gabapentin 200mg + Nortriptyline 10mg
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="druglist-item_fullDetailsContainer__RCxNm">
                <div
                  className="druglist-item_substituteHeaderContainer__ezxIX"
                  style={{ width: "100%" }}
                >
                  <div className="druglist-item_substituteMedicineDetails__0wLPq">
                    <div
                      style={{
                        display: "flex",
                        justifyContent: "space-between",
                        flexDirection: "row",
                      }}
                    >
                      <div className="druglist-item_genericContentCont__mg7Dq">
                        <div
                          className="druglist-item_imgContainer__3MOmx"
                          style={{
                            display: "flex",
                            flexFlow: "column",
                            alignItems: "flex-start",
                            justifyContent: "space-between",
                          }}
                        >
                          <div
                            className="druglist-item_itemImg__KBvIo"
                            style={{
                              backgroundImage:
                                "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                            }}
                          ></div>
                        </div>
                        <div className="druglist-item_itemDetails__JhI3f">
                          <div className="druglist-item_itemName__YnmLq">
                            Doloneuron NT 200/10mg Tab 10s
                          </div>
                          <div className="druglist-item_substituteManufacturerName__fECQK">
                            Pulse Pharmaceuticals
                          </div>
                          <div className="druglist-item_Quantity__y1rmm">
                            Strip of 10 tablets
                          </div>
                          <div
                            className=" "
                            style={{
                              display: "flex",
                              justifyContent: "space-between",
                              alignItems: "flex-end",
                            }}
                          >
                            <div className="druglist-item_substitutePriceContainer__75Y7h">
                              <div className="druglist-item_substitutePrice__Qdsht">
                                <p className="druglist-item_discountedPrice__ftOvo">
                                  ₹143.1
                                </p>
                                <div className="druglist-item_discountChip__qAfw_">
                                  <div>
                                    <svg
                                      width="20"
                                      height="20"
                                      viewBox="0 0 20 20"
                                      xmlns="http://www.w3.org/2000/svg"
                                      style={{ verticalAlign: "text-bottom" }}
                                    >
                                      <path
                                        d="M17.2911 10L18.2111 8.41001C18.3217 8.21871 18.3517 7.99133 18.2947 7.77786C18.2377 7.5644 18.0982 7.38232 17.9069 7.27167L16.3153 6.35167V4.51834C16.3153 4.29733 16.2275 4.08537 16.0712 3.92909C15.9149 3.7728 15.703 3.68501 15.4819 3.68501H13.6494L12.7303 2.09417C12.6193 1.90327 12.4375 1.76376 12.2244 1.70584C12.1187 1.67719 12.0084 1.6698 11.8998 1.6841C11.7913 1.69841 11.6866 1.73412 11.5919 1.78917L10.0003 2.70917L8.40861 1.78834C8.21721 1.67784 7.98975 1.64789 7.77627 1.70509C7.5628 1.76229 7.38078 1.90195 7.27027 2.09334L6.35027 3.68501H4.51777C4.29676 3.68501 4.0848 3.7728 3.92852 3.92909C3.77224 4.08537 3.68444 4.29733 3.68444 4.51834V6.35084L2.09277 7.27084C1.9978 7.32547 1.91456 7.39833 1.84783 7.48523C1.7811 7.57212 1.7322 7.67135 1.70393 7.77721C1.67567 7.88306 1.6686 7.99346 1.68313 8.10206C1.69766 8.21065 1.73351 8.31531 1.78861 8.41001L2.70861 10L1.78861 11.59C1.6786 11.7815 1.64875 12.0087 1.70557 12.2221C1.76239 12.4355 1.90128 12.6178 2.09194 12.7292L3.68361 13.6492V15.4817C3.68361 15.7027 3.7714 15.9146 3.92768 16.0709C4.08396 16.2272 4.29593 16.315 4.51694 16.315H6.35027L7.27027 17.9067C7.34405 18.0328 7.44939 18.1375 7.57592 18.2106C7.70246 18.2836 7.84583 18.3225 7.99194 18.3233C8.13694 18.3233 8.28111 18.285 8.40944 18.2108L9.99944 17.2908L11.5911 18.2108C11.7825 18.3212 12.0098 18.3511 12.2232 18.2941C12.4366 18.2371 12.6186 18.0978 12.7294 17.9067L13.6486 16.315H15.4811C15.7021 16.315 15.9141 16.2272 16.0704 16.0709C16.2266 15.9146 16.3144 15.7027 16.3144 15.4817V13.6492L17.9061 12.7292C18.0009 12.6744 18.084 12.6014 18.1506 12.5145C18.2172 12.4276 18.266 12.3284 18.2943 12.2226C18.3225 12.1168 18.3296 12.0065 18.3152 11.898C18.3008 11.7894 18.2652 11.6848 18.2103 11.59L17.2911 10ZM7.91611 5.82501C8.24774 5.82512 8.56574 5.95696 8.80016 6.19154C9.03458 6.42612 9.16622 6.74421 9.16611 7.07584C9.166 7.40747 9.03415 7.72548 8.79957 7.9599C8.565 8.19432 8.2469 8.32595 7.91527 8.32584C7.58364 8.32573 7.26564 8.19388 7.03122 7.95931C6.7968 7.72473 6.66516 7.40664 6.66527 7.07501C6.66538 6.74338 6.79723 6.42537 7.03181 6.19095C7.26638 5.95653 7.58448 5.8249 7.91611 5.82501ZM8.16611 13.825L6.83277 12.8258L11.8328 6.15917L13.1661 7.15834L8.16611 13.825ZM12.0828 14.1583C11.9186 14.1583 11.756 14.1259 11.6043 14.063C11.4526 14.0001 11.3148 13.908 11.1987 13.7918C11.0826 13.6757 10.9906 13.5378 10.9278 13.3861C10.865 13.2343 10.8327 13.0717 10.8328 12.9075C10.8328 12.7433 10.8652 12.5807 10.9281 12.429C10.991 12.2773 11.0832 12.1395 11.1993 12.0235C11.3155 11.9074 11.4533 11.8153 11.6051 11.7525C11.7568 11.6897 11.9194 11.6575 12.0836 11.6575C12.4152 11.6576 12.7332 11.7895 12.9677 12.024C13.2021 12.2586 13.3337 12.5767 13.3336 12.9083C13.3335 13.24 13.2016 13.558 12.9671 13.7924C12.7325 14.0268 12.4144 14.1585 12.0828 14.1583Z"
                                        fill="#fff"
                                      ></path>
                                    </svg>
                                  </div>
                                  <div>18% OFF</div>
                                </div>
                              </div>
                              <p className="druglist-item_itemMrp__aSi9J">
                                MRP ₹<span>174.5</span>
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          justifyContent: "flex-end",
                          alignItems: "flex-end",
                          position: "relative",
                        }}
                      ></div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </a>
          <div className="product-listing_divider__aiMpd"></div>
          <a href="/product-display/dolo-soft-mr-100mg-2mg-tablet/1192316">
            <div className="druglist-item_itemContainer__Uqt1t">
              <div className="druglist-item_itemHeading__cWGyl">
                <div className="druglist-item_saltMobileContainer__0R4yP">
                  <div className="druglist-item_saltMobileContainer__content__ILi6H">
                    <div className="druglist-item_saltMobileContainer__title__3zs9s">
                      <img
                        alt="Molecule"
                        loading="lazy"
                        width="32"
                        height="32"
                        decoding="async"
                        className="druglist-item_saltMobileContainer__icon__L13mN"
                        srcSet={`
                            ${saltImage}?w=32&amp;q=80 1x,
                            ${saltImage}?w=64&amp;q=80 2x
                          `}
                        src={`${saltImage}?w=64&amp;q=80`}
                        style={{ color: "transparent", display: "none" }}
                      />
                      <div className="druglist-item_saltMobileContainer__titleText__9UmPF">
                        Salt Composition
                      </div>
                    </div>
                    <div className="druglist-item_saltMobileContainer__composition__EsLOi">
                      <p className="druglist-item_saltMobileContainer__composition__less_salt__B_Gip">
                        Nimesulide 100mg + Tizanidine 2mg
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="druglist-item_fullDetailsContainer__RCxNm">
                <div
                  className="druglist-item_substituteHeaderContainer__ezxIX"
                  style={{ width: "100%" }}
                >
                  <div
                    className="druglist-item_substituteMedicineDetails__0wLPq"
                    style={{ backgroundColor: "rgb(250, 250, 250)" }}
                  >
                    <div
                      style={{
                        display: "flex",
                        justifyContent: "space-between",
                        flexDirection: "row",
                      }}
                    >
                      <div className="druglist-item_genericContentCont__mg7Dq">
                        <div
                          className="druglist-item_imgContainer__3MOmx"
                          style={{
                            display: "flex",
                            flexFlow: "column",
                            alignItems: "flex-start",
                            justifyContent: "space-between",
                          }}
                        >
                          <div
                            className="druglist-item_itemImg__KBvIo"
                            style={{
                              backgroundImage:
                                "url('https://platinumrxassets.blob.core.windows.net/platinumrx-assets/default_image.jpg')",
                            }}
                          ></div>
                        </div>
                        <div className="druglist-item_itemDetails__JhI3f">
                          <div className="druglist-item_itemName__YnmLq">
                            Dolo Soft MR 100mg/2mg Tablet
                          </div>
                          <div className="druglist-item_substituteManufacturerName__fECQK">
                            Gujarat Liqui Pharmacaps Pvt Ltd
                          </div>
                          <div className="druglist-item_Quantity__y1rmm">
                            Strip of 10 tablets
                          </div>
                          <div
                            className=" "
                            style={{
                              display: "flex",
                              justifyContent: "space-between",
                              alignItems: "flex-end",
                            }}
                          >
                            <div className="druglist-item_substitutePriceContainer__75Y7h">
                              <div className="druglist-item_substitutePrice__Qdsht"></div>
                              <p className="druglist-item_itemMrp__aSi9J">
                                MRP ₹47
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          justifyContent: "flex-end",
                          alignItems: "flex-end",
                          position: "relative",
                        }}
                      ></div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </a>
          <div className="product-listing_divider__aiMpd"></div>
          <a href="/product-display/dolo-t-37-5mg-325mg-tablet/1002220">
            <div className="druglist-item_itemContainer__Uqt1t">
              <div className="druglist-item_itemHeading__cWGyl">
                <div className="druglist-item_saltMobileContainer__0R4yP">
                  <div className="druglist-item_saltMobileContainer__content__ILi6H">
                    <div className="druglist-item_saltMobileContainer__title__3zs9s">
                      <img
                        alt="Molecule"
                        loading="lazy"
                        width="32"
                        height="32"
                        decoding="async"
                        className="druglist-item_saltMobileContainer__icon__L13mN"
                        srcSet={`
                            ${saltImage}?w=32&amp;q=80 1x,
                            ${saltImage}?w=64&amp;q=80 2x
                          `}
                        src={`${saltImage}?w=64&amp;q=80`}
                        style={{ color: "transparent", display: "none" }}
                      />
                      <div className="druglist-item_saltMobileContainer__titleText__9UmPF">
                        Salt Composition
                      </div>
                    </div>
                    <div className="druglist-item_saltMobileContainer__composition__EsLOi">
                      <p className="druglist-item_saltMobileContainer__composition__less_salt__B_Gip">
                        Paracetamol (Acetaminophen) 325mg + Tramadol 37.5mg
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="druglist-item_fullDetailsContainer__RCxNm">
                <div
                  className="druglist-item_substituteHeaderContainer__ezxIX"
                  style={{ width: "100%" }}
                >
                  <div
                    className="druglist-item_substituteMedicineDetails__0wLPq"
                    style={{ backgroundColor: "rgb(250, 250, 250)" }}
                  >
                    <div
                      style={{
                        display: "flex",
                        justifyContent: "space-between",
                        flexDirection: "row",
                      }}
                    >
                      <div className="druglist-item_genericContentCont__mg7Dq">
                        <div
                          className="druglist-item_imgContainer__3MOmx"
                          style={{
                            display: "flex",
                            flexFlow: "column",
                            alignItems: "flex-start",
                            justifyContent: "space-between",
                          }}
                        >
                          <div
                            className="druglist-item_itemImg__KBvIo"
                            style={{
                              backgroundImage:
                                "url('https://platinumrxassets.blob.core.windows.net/platinumrx-assets/default_image.jpg')",
                            }}
                          ></div>
                        </div>
                        <div className="druglist-item_itemDetails__JhI3f">
                          <div className="druglist-item_itemName__YnmLq">
                            Dolo T 37.5mg/325mg Tablet
                          </div>
                          <div className="druglist-item_substituteManufacturerName__fECQK">
                            Micro Labs Ltd
                          </div>
                          <div className="druglist-item_Quantity__y1rmm">
                            Strip of 10 tablets
                          </div>
                          <div
                            className=" "
                            style={{
                              display: "flex",
                              justifyContent: "space-between",
                              alignItems: "flex-end",
                            }}
                          >
                            <div className="druglist-item_substitutePriceContainer__75Y7h">
                              <div className="druglist-item_substitutePrice__Qdsht"></div>
                              <p className="druglist-item_itemMrp__aSi9J">
                                MRP ₹113
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          justifyContent: "flex-end",
                          alignItems: "flex-end",
                          position: "relative",
                        }}
                      ></div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </a>
          <div className="product-listing_divider__aiMpd"></div>
          <a href="/product-display/dolo-t-tablet/1289165">
            <div className="druglist-item_itemContainer__Uqt1t">
              <div className="druglist-item_itemHeading__cWGyl">
                <div className="druglist-item_saltMobileContainer__0R4yP">
                  <div className="druglist-item_saltMobileContainer__content__ILi6H">
                    <div className="druglist-item_saltMobileContainer__title__3zs9s">
                      <img
                        alt="Molecule"
                        loading="lazy"
                        width="32"
                        height="32"
                        decoding="async"
                        className="druglist-item_saltMobileContainer__icon__L13mN"
                        srcSet={`
                            ${saltImage}?w=32&amp;q=80 1x,
                            ${saltImage}?w=64&amp;q=80 2x
                          `}
                        src={`${saltImage}?w=64&amp;q=80`}
                        style={{ color: "transparent", display: "none" }}
                      />
                      <div className="druglist-item_saltMobileContainer__titleText__9UmPF">
                        Salt Composition
                      </div>
                    </div>
                    <div className="druglist-item_saltMobileContainer__composition__EsLOi">
                      <p className="druglist-item_saltMobileContainer__composition__less_salt__B_Gip">
                        Paracetamol (Acetaminophen) 325mg + Tramadol 37.5mg
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="druglist-item_fullDetailsContainer__RCxNm">
                <div
                  className="druglist-item_substituteHeaderContainer__ezxIX"
                  style={{ width: "100%" }}
                >
                  <div
                    className="druglist-item_substituteMedicineDetails__0wLPq"
                    style={{ backgroundColor: "rgb(250, 250, 250)" }}
                  >
                    <div
                      style={{
                        display: "flex",
                        justifyContent: "space-between",
                        flexDirection: "row",
                      }}
                    >
                      <div className="druglist-item_genericContentCont__mg7Dq">
                        <div
                          className="druglist-item_imgContainer__3MOmx"
                          style={{
                            display: "flex",
                            flexFlow: "column",
                            alignItems: "flex-start",
                            justifyContent: "space-between",
                          }}
                        >
                          <div
                            className="druglist-item_itemImg__KBvIo"
                            style={{
                              backgroundImage:
                                "url('https://platinumrxassets.blob.core.windows.net/platinumrx-assets/default_image.jpg')",
                            }}
                          ></div>
                        </div>
                        <div className="druglist-item_itemDetails__JhI3f">
                          <div className="druglist-item_itemName__YnmLq">
                            Dolo-T Tablet
                          </div>
                          <div className="druglist-item_substituteManufacturerName__fECQK">
                            Micro Labs Ltd
                          </div>
                          <div className="druglist-item_Quantity__y1rmm">
                            Strip of 10 tablets
                          </div>
                          <div
                            className=" "
                            style={{
                              display: "flex",
                              justifyContent: "space-between",
                              alignItems: "flex-end",
                            }}
                          >
                            <div className="druglist-item_substitutePriceContainer__75Y7h">
                              <div className="druglist-item_substitutePrice__Qdsht"></div>
                              <p className="druglist-item_itemMrp__aSi9J">
                                MRP ₹123
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          justifyContent: "flex-end",
                          alignItems: "flex-end",
                          position: "relative",
                        }}
                      ></div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </a>
          <div className="product-listing_divider__aiMpd"></div>
          <a href="/product-display/dolo-cold-500mg-30mg-10mg-tablet/1095549">
            <div className="druglist-item_itemContainer__Uqt1t">
              <div className="druglist-item_itemHeading__cWGyl">
                <div className="druglist-item_saltMobileContainer__0R4yP">
                  <div className="druglist-item_saltMobileContainer__content__ILi6H">
                    <div className="druglist-item_saltMobileContainer__title__3zs9s">
                      <img
                        alt="Molecule"
                        loading="lazy"
                        width="32"
                        height="32"
                        decoding="async"
                        className="druglist-item_saltMobileContainer__icon__L13mN"
                        srcSet={`
                            ${saltImage}?w=32&amp;q=80 1x,
                            ${saltImage}?w=64&amp;q=80 2x
                          `}
                        src={`${saltImage}?w=64&amp;q=80`}
                        style={{ color: "transparent", display: "none" }}
                      />
                      <div className="druglist-item_saltMobileContainer__titleText__9UmPF">
                        Salt Composition
                      </div>
                    </div>
                    <div className="druglist-item_saltMobileContainer__composition__EsLOi">
                      <p className="druglist-item_saltMobileContainer__composition__less_salt__B_Gip">
                        Caffeine 30mg + Paracetamol (Acetaminophen) 500mg +
                        Phenylephrine 10mg
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="druglist-item_fullDetailsContainer__RCxNm">
                <div className="druglist-item_genericMedicineDetails__rdmT5">
                  <div
                    style={{
                      display: "flex",
                      justifyContent: "space-between",
                      flexDirection: "row",
                    }}
                  >
                    <div className="druglist-item_contentContainer__OyI1B">
                      <div
                        className="druglist-item_imgContainer__3MOmx"
                        style={{
                          display: "flex",
                          alignItems: "flex-end",
                          flexWrap: "nowrap",
                        }}
                      >
                        <div
                          className="druglist-item_itemImg__KBvIo"
                          style={{
                            backgroundImage:
                              "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                          }}
                        ></div>
                      </div>
                      <div
                        className="druglist-item_itemDetails__JhI3f"
                        style={{ marginTop: "8px" }}
                      >
                        <div className="druglist-item_itemName__YnmLq">
                          Dolo Cold 500mg/30mg/10mg Tablet
                        </div>
                        <div className="druglist-item_genericManufacturerName__Du3NS">
                          Micro Labs Ltd
                        </div>
                        <div className="druglist-item_Quantity__y1rmm">
                          Strip of 10 tablets
                        </div>
                        <div className="druglist-item_itemMrp__aSi9J">MRP</div>
                        <p className="druglist-item_genericPrice__0u9Ol">₹35</p>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="druglist-item_noItemContainer__bja8_">
                  <div className="NoItemComponent_noitem__K_T3j">
                    <div
                      className="NoItemComponent_itemImgPLP__7nKbp"
                      style={{
                        backgroundImage:
                          "url('/_next/static/media/default-med-find.c3b30ad6.png')",
                        backgroundSize: "70%",
                      }}
                    ></div>
                    <div
                      className="NoItemComponent_noitem__content__qfxOg"
                      style={{ paddingLeft: "0px", width: "100%" }}
                    >
                      <div className="NoItemComponent_NoitemHeading__JGEmh">
                        Sorry, the substitute medicine is Currently Unavailable
                      </div>
                      <div
                        style={{
                          width: "100%",
                          marginTop: "8px",
                          alignSelf: "flex-end",
                        }}
                      ></div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </a>
          <div className="product-listing_divider__aiMpd"></div>
          <a href="/product-display/dolo-156-25mg-suspension/1102873">
            <div className="druglist-item_itemContainer__Uqt1t">
              <div className="druglist-item_itemHeading__cWGyl">
                <div className="druglist-item_saltMobileContainer__0R4yP">
                  <div className="druglist-item_saltMobileContainer__content__ILi6H">
                    <div className="druglist-item_saltMobileContainer__title__3zs9s">
                      <img
                        alt="Molecule"
                        loading="lazy"
                        width="32"
                        height="32"
                        decoding="async"
                        className="druglist-item_saltMobileContainer__icon__L13mN"
                        srcSet={`
                            ${saltImage}?w=32&amp;q=80 1x,
                            ${saltImage}?w=64&amp;q=80 2x
                          `}
                        src={`${saltImage}?w=64&amp;q=80`}
                        style={{ color: "transparent", display: "none" }}
                      />
                      <div className="druglist-item_saltMobileContainer__titleText__9UmPF">
                        Salt Composition
                      </div>
                    </div>
                    <div className="druglist-item_saltMobileContainer__composition__EsLOi">
                      <p className="druglist-item_saltMobileContainer__composition__less_salt__B_Gip">
                        Paracetamol (Acetaminophen) 150mg
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="druglist-item_fullDetailsContainer__RCxNm">
                <div className="druglist-item_genericMedicineDetails__rdmT5">
                  <div
                    style={{
                      display: "flex",
                      justifyContent: "space-between",
                      flexDirection: "row",
                    }}
                  >
                    <div className="druglist-item_contentContainer__OyI1B">
                      <div
                        className="druglist-item_imgContainer__3MOmx"
                        style={{
                          display: "flex",
                          alignItems: "flex-end",
                          flexWrap: "nowrap",
                        }}
                      >
                        <div
                          className="druglist-item_itemImg__KBvIo"
                          style={{
                            backgroundImage:
                              "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                          }}
                        ></div>
                      </div>
                      <div
                        className="druglist-item_itemDetails__JhI3f"
                        style={{ marginTop: "8px" }}
                      >
                        <div className="druglist-item_itemName__YnmLq">
                          Dolo 156.25mg Suspension
                        </div>
                        <div className="druglist-item_genericManufacturerName__Du3NS">
                          Micro Labs Ltd
                        </div>
                        <div className="druglist-item_Quantity__y1rmm">
                          Bottle of 60 ml
                        </div>
                        <div className="druglist-item_itemMrp__aSi9J">MRP</div>
                        <p className="druglist-item_genericPrice__0u9Ol">₹56</p>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="druglist-item_noItemContainer__bja8_">
                  <div className="NoItemComponent_noitem__K_T3j">
                    <div
                      className="NoItemComponent_itemImgPLP__7nKbp"
                      style={{
                        backgroundImage:
                          "url('/_next/static/media/default-med-find.c3b30ad6.png')",
                        backgroundSize: "70%",
                      }}
                    ></div>
                    <div
                      className="NoItemComponent_noitem__content__qfxOg"
                      style={{ paddingLeft: "0px", width: "100%" }}
                    >
                      <div className="NoItemComponent_NoitemHeading__JGEmh">
                        Sorry, the substitute medicine is Currently Unavailable
                      </div>
                      <div
                        style={{
                          width: "100%",
                          marginTop: "8px",
                          alignSelf: "flex-end",
                        }}
                      ></div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </a>
          <div className="product-listing_divider__aiMpd"></div>
          <a href="/product-display/dolop-plus-325mg-37-5mg-tablet/1220985">
            <div className="druglist-item_itemContainer__Uqt1t">
              <div className="druglist-item_itemHeading__cWGyl">
                <div className="druglist-item_saltMobileContainer__0R4yP">
                  <div className="druglist-item_saltMobileContainer__content__ILi6H">
                    <div className="druglist-item_saltMobileContainer__title__3zs9s">
                      <img
                        alt="Molecule"
                        loading="lazy"
                        width="32"
                        height="32"
                        decoding="async"
                        className="druglist-item_saltMobileContainer__icon__L13mN"
                        srcSet={`
                            ${saltImage}?w=32&amp;q=80 1x,
                            ${saltImage}?w=64&amp;q=80 2x
                          `}
                        src={`${saltImage}?w=64&amp;q=80`}
                        style={{ color: "transparent", display: "none" }}
                      />
                      <div className="druglist-item_saltMobileContainer__titleText__9UmPF">
                        Salt Composition
                      </div>
                    </div>
                    <div className="druglist-item_saltMobileContainer__composition__EsLOi">
                      <p className="druglist-item_saltMobileContainer__composition__less_salt__B_Gip">
                        Paracetamol (Acetaminophen) 325mg + Tramadol 37.5mg
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="druglist-item_fullDetailsContainer__RCxNm">
                <div
                  className="druglist-item_substituteHeaderContainer__ezxIX"
                  style={{ width: "100%" }}
                >
                  <div
                    className="druglist-item_substituteMedicineDetails__0wLPq"
                    style={{ backgroundColor: "rgb(250, 250, 250)" }}
                  >
                    <div
                      style={{
                        display: "flex",
                        justifyContent: "space-between",
                        flexDirection: "row",
                      }}
                    >
                      <div className="druglist-item_genericContentCont__mg7Dq">
                        <div
                          className="druglist-item_imgContainer__3MOmx"
                          style={{
                            display: "flex",
                            flexFlow: "column",
                            alignItems: "flex-start",
                            justifyContent: "space-between",
                          }}
                        >
                          <div
                            className="druglist-item_itemImg__KBvIo"
                            style={{
                              backgroundImage:
                                "url('https://platinumrxassets.blob.core.windows.net/platinumrx-assets/default_image.jpg')",
                            }}
                          ></div>
                        </div>
                        <div className="druglist-item_itemDetails__JhI3f">
                          <div className="druglist-item_itemName__YnmLq">
                            Dolop Plus 325mg/37.5mg Tablet
                          </div>
                          <div className="druglist-item_substituteManufacturerName__fECQK">
                            Aurum Life Science Pvt Ltd
                          </div>
                          <div className="druglist-item_Quantity__y1rmm">
                            Strip of 10 tablets
                          </div>
                          <div
                            className=" "
                            style={{
                              display: "flex",
                              justifyContent: "space-between",
                              alignItems: "flex-end",
                            }}
                          >
                            <div className="druglist-item_substitutePriceContainer__75Y7h">
                              <div className="druglist-item_substitutePrice__Qdsht"></div>
                              <p className="druglist-item_itemMrp__aSi9J">
                                MRP ₹9.5
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          justifyContent: "flex-end",
                          alignItems: "flex-end",
                          position: "relative",
                        }}
                      ></div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </a>
          <div className="product-listing_divider__aiMpd"></div>
          <a href="/product-display/new-dolo-cold-tablet/1121483">
            <div className="druglist-item_itemContainer__Uqt1t">
              <div className="druglist-item_itemHeading__cWGyl">
                <div className="druglist-item_saltMobileContainer__0R4yP">
                  <div className="druglist-item_saltMobileContainer__content__ILi6H">
                    <div className="druglist-item_saltMobileContainer__title__3zs9s">
                      <img
                        alt="Molecule"
                        loading="lazy"
                        width="32"
                        height="32"
                        decoding="async"
                        className="druglist-item_saltMobileContainer__icon__L13mN"
                        srcSet={`
                            ${saltImage}?w=32&amp;q=80 1x,
                            ${saltImage}?w=64&amp;q=80 2x
                          `}
                        src={`${saltImage}?w=64&amp;q=80`}
                        style={{ color: "transparent", display: "none" }}
                      />
                      <div className="druglist-item_saltMobileContainer__titleText__9UmPF">
                        Salt Composition
                      </div>
                    </div>
                    <div className="druglist-item_saltMobileContainer__composition__EsLOi">
                      <p className="druglist-item_saltMobileContainer__composition__less_salt__B_Gip">
                        Caffeine 30mg + Chlorpheniramine Maleate 2mg +
                        Paracetamol (Acetaminophen) 325mg + Phenylephrine 10mg
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="druglist-item_fullDetailsContainer__RCxNm">
                <div
                  className="druglist-item_substituteHeaderContainer__ezxIX"
                  style={{ width: "100%" }}
                >
                  <div
                    className="druglist-item_substituteMedicineDetails__0wLPq"
                    style={{ backgroundColor: "rgb(250, 250, 250)" }}
                  >
                    <div
                      style={{
                        display: "flex",
                        justifyContent: "space-between",
                        flexDirection: "row",
                      }}
                    >
                      <div className="druglist-item_genericContentCont__mg7Dq">
                        <div
                          className="druglist-item_imgContainer__3MOmx"
                          style={{
                            display: "flex",
                            flexFlow: "column",
                            alignItems: "flex-start",
                            justifyContent: "space-between",
                          }}
                        >
                          <div
                            className="druglist-item_itemImg__KBvIo"
                            style={{
                              backgroundImage:
                                "url('https://platinumrxassets.blob.core.windows.net/platinumrx-assets/default_image.jpg')",
                            }}
                          ></div>
                        </div>
                        <div className="druglist-item_itemDetails__JhI3f">
                          <div className="druglist-item_itemName__YnmLq">
                            New Dolo-Cold Tablet
                          </div>
                          <div className="druglist-item_substituteManufacturerName__fECQK">
                            Micro Labs Ltd
                          </div>
                          <div className="druglist-item_Quantity__y1rmm">
                            Strip of 10 tablets
                          </div>
                          <div
                            className=" "
                            style={{
                              display: "flex",
                              justifyContent: "space-between",
                              alignItems: "flex-end",
                            }}
                          >
                            <div className="druglist-item_substitutePriceContainer__75Y7h">
                              <div className="druglist-item_substitutePrice__Qdsht"></div>
                              <p className="druglist-item_itemMrp__aSi9J">
                                MRP ₹35
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          justifyContent: "flex-end",
                          alignItems: "flex-end",
                          position: "relative",
                        }}
                      ></div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </a>
          <div className="product-listing_divider__aiMpd"></div>
          <a href="/product-display/nvm-dolo-tablet-sr/1220923">
            <div className="druglist-item_itemContainer__Uqt1t">
              <div className="druglist-item_itemHeading__cWGyl">
                <div className="druglist-item_saltMobileContainer__0R4yP">
                  <div className="druglist-item_saltMobileContainer__content__ILi6H">
                    <div className="druglist-item_saltMobileContainer__title__3zs9s">
                      <img
                        alt="Molecule"
                        loading="lazy"
                        width="32"
                        height="32"
                        decoding="async"
                        className="druglist-item_saltMobileContainer__icon__L13mN"
                        srcSet={`
                            ${saltImage}?w=32&amp;q=80 1x,
                            ${saltImage}?w=64&amp;q=80 2x
                          `}
                        src={`${saltImage}?w=64&amp;q=80`}
                        style={{ color: "transparent", display: "none" }}
                      />
                      <div className="druglist-item_saltMobileContainer__titleText__9UmPF">
                        Salt Composition
                      </div>
                    </div>
                    <div className="druglist-item_saltMobileContainer__composition__EsLOi">
                      <p className="druglist-item_saltMobileContainer__composition__less_salt__B_Gip">
                        Pregabalin 75mg + Vitamin B12 (Methylcobalamin) 1500mcg
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="druglist-item_fullDetailsContainer__RCxNm">
                <div
                  className="druglist-item_substituteHeaderContainer__ezxIX"
                  style={{ width: "100%" }}
                >
                  <div
                    className="druglist-item_substituteMedicineDetails__0wLPq"
                    style={{ backgroundColor: "rgb(250, 250, 250)" }}
                  >
                    <div
                      style={{
                        display: "flex",
                        justifyContent: "space-between",
                        flexDirection: "row",
                      }}
                    >
                      <div className="druglist-item_genericContentCont__mg7Dq">
                        <div
                          className="druglist-item_imgContainer__3MOmx"
                          style={{
                            display: "flex",
                            flexFlow: "column",
                            alignItems: "flex-start",
                            justifyContent: "space-between",
                          }}
                        >
                          <div
                            className="druglist-item_itemImg__KBvIo"
                            style={{
                              backgroundImage:
                                "url('https://flexipillstorage.blob.core.windows.net/meds-prescriptions/1544e6ad-26a2-40cc-8eb0-f57f735be98d.jpeg')",
                            }}
                          ></div>
                        </div>
                        <div className="druglist-item_itemDetails__JhI3f">
                          <div className="druglist-item_itemName__YnmLq">
                            Nvm Dolo Tablet SR
                          </div>
                          <div className="druglist-item_substituteManufacturerName__fECQK">
                            Seagull Pharmaceutical Pvt Ltd
                          </div>
                          <div className="druglist-item_Quantity__y1rmm">
                            Strip of 10 tablets
                          </div>
                          <div
                            className=" "
                            style={{
                              display: "flex",
                              justifyContent: "space-between",
                              alignItems: "flex-end",
                            }}
                          >
                            <div className="druglist-item_substitutePriceContainer__75Y7h">
                              <div className="druglist-item_substitutePrice__Qdsht"></div>
                              <p className="druglist-item_itemMrp__aSi9J">
                                MRP ₹100
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          justifyContent: "flex-end",
                          alignItems: "flex-end",
                          position: "relative",
                        }}
                      ></div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </a>
          <div className="product-listing_divider__aiMpd"></div>
        </div>
      </div>
    </section>
  );
};

export default NameSearchPageDemo;
